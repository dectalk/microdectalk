...
int all_phsort(LPTTS_HANDLE_T phTTS) {
	...
	for(n = 0; n < pDph_t->nsymbtot; n++) {
		...
		if(pKsd_t->lang_curr == LANG_german) {
			if((n > 1) && (pDph_t->symbols[n]) >= WBOUND && (pDph_t->symbols[n] <= RELSTART))
					...
					if((pDph_t->symbols[nextthing] & PVALUE) < 100) {
						...
						if((pDph_t->symbols[n - 1] == pDph_t->symbols[nextthing]) /* RSM */
						   || (phone_feature(pDph_t, pDph_t->symbols[n - 1]) & FVOICD) &&
							  (phone_feature(pDph_t, pDph_t->symbols[nextthing]) & FVOICD) && !(phone_feature(pDph_t, pDph_t->symbols[nextthing]) & FSTOP)) {
		...
		if(pDph_t->symbols[n] == NEW_PARAGRAPH && pDph_t->symbols[n + 1] == NEW_PARAGRAPH) {
		...
		if(pDph_t->symbols[n] == LAP_NH || pDph_t->symbols[n] == LAP_LL)
		...
		if(pDph_t->symbols[n] == SPP_NH || pDph_t->symbols[n] == SPP_LL)
		...
		if(pKsd_t->lang_curr == LANG_spanish) {
			...
			if(pDph_t->symbols[n] == WBOUND || n == 0) {
				...
				if((pDph_t->symbols[n + 1] == SPP_I) && (pDph_t->symbols[n + 2] == WBOUND)) {
				...
				/* allophonic rule for "or" O->e
				   before an I as in Y india pronounced e india*/
				if((pDph_t->symbols[n + 2] == SPP_O) && (pDph_t->symbols[n + 3] == WBOUND)) {
				...
				if(pDph_t->symbols[n - 1] == SPP_S || pDph_t->symbols[n - 1] == SPP_TH) {
					...
					if((pDph_t->symbols[ntmp]) == SPP_S || (pDph_t->symbols[ntmp]) == SPP_TH) {
						...
						/* BATS 677 Found another spot it was doing it wrong
						   EAB 5/18/98 */
						if(!(pDph_t->wordclass[n - 1] & 0x00800000) && !(pDph_t->wordclass[n + 1] & 0x00800000)) {
							insertphone(phTTS, n, SPP_Q);
							/* 12/10/1996 EDB */
							n = ntmp;
						}
						...
						/* BATS 677 Found another spot it was doing it wrong
						   ...
						   marker with no silence at the end yet */
							...
							if((pDph_t->symbols[n + 1] & PVALUE) != GEN_SIL)
								if(!((pDph_t->symbols[n + 1] & PVALUE) >= 115 && (pDph_t->symbols[n + 1] & PVALUE) <= 118))
									insertphone(phTTS, n, SPP_Q);
		...
		if(pKsd_t->lang_curr == LANG_latin_american) {
			...
			if(pDph_t->symbols[n] == WBOUND || n == 0) {
				/* allophonic rule for grapheme Y->e
				   before an I as in Y india pronounced e india */
				if((pDph_t->symbols[n + 1] == LAP_I) && (pDph_t->symbols[n + 2] == WBOUND)) {
					/*then look ahead to see if the
				...
				/* allophonic rule for "or" O->e
				   before an I as in Y india pronounced e india*/
				if((pDph_t->symbols[n + 2] == LAP_O) && (pDph_t->symbols[n + 3] == WBOUND)) {
		...
		if(pKsd_t->lang_curr == LANG_latin_american || pKsd_t->lang_curr == LANG_spanish) {
			...
			{
				if((pDph_t->symbols[n - 1] & PVALUE) == WBOUND && (pDph_t->symbols[n + 1] & PVALUE) == WBOUND) {
...
#ifdef ENGLISH_US
					if((pDph_t->symbols[m] & PVALUE) > WBOUND && (pDph_t->symbols[m] & PVALUE) < NEW_PARAGRAPH && (pDph_t->symbols[m] & PVALUE) != HYPHEN /* xxx for auto compunds */)
#endif
	...
	for(n = 0; n < pDph_t->nsymbtot; n++) {
		...
		if(curr_in_sym < MAX_PHONES) {					 /* A real phoneme */
...
static void interp_user_f0(PDPH_T pDph_t, short* psCurr_dur, short* psCurr_f0,
	...
	if(((curr_in_sym == S1) || (curr_in_sym == SEMPH) || (curr_in_sym == HAT_RISE) || (curr_in_sym == HAT_FALL)) && (pDph_t->f0mode != PHONE_TARGETS_SPECIFIED) && (pDph_t->f0mode != SINGING)) {
	...
	/* Otherwise, they are note commands for singing, or f0 targets for segs */
		...
		if(pDph_t->f0mode != HAT_F0_SIZES_SPECIFIED) {
			if((pDph_t->f0mode != PHONE_TARGETS_SPECIFIED) && ((*psCurr_f0 % 1000) <= 37)) {
				pDph_t->f0mode = SINGING;

			...
			if((!pKsd_t->halting) && (!phTTS->bMemoryReset)) {
				...
				*/
...
#ifdef HLSYN
				if(pVtm_t->parambuff[OUT_A2 + 1] == 2000 && pVtm_t->llframe.NAF > 40)
					pVtm_t->in_region += 1;
				else
					pVtm_t->in_region = 0;
				...
				if(DT_DBG(PH_DBG, 0x040)) {
					...
					if(pVtm_t->llframe.NAF > 0 || pVtm_t->llframe.NAH > 0 || pVtm_t->llframe.NAV > 0) {
						printf(" AF %d ", pVtm_t->llframe.NAF);
						printf(" AH %d \n", pVtm_t->llframe.NAH);
						printf(" AV %d \n", pVtm_t->llframe.NAV);
						if(pVtm_t->llframe.NA2F + pVtm_t->llframe.NA3F + pVtm_t->llframe.NA4F +
...
//		      }
		...
		case SPC_type_tone:
			...
			if(!pKsd_t->halting) { /*The elements of pVtm_t are used ,not been modified in
		...
		case SPC_type_index:
			...
			if(!pKsd_t->halting) {
				...
				case STATE_OUTPUT_AUDIO:
					...
					if((pVtm_t->control == (SPC_type_index | SPC_subtype_bookmark)) || (pVtm_t->control == (SPC_type_index | SPC_subtype_wordpos)) || (pVtm_t->control == (SPC_type_index | SPC_subtype_start)) || (pVtm_t->control == (SPC_type_index | SPC_subtype_sentence)) || (pVtm_t->control == (SPC_type_index | SPC_subtype_stop)) || (pVtm_t->control == (SPC_type_index | SPC_subtype_volume))) {
...
void OutputData(LPTTS_HANDLE_T phTTS,
	...
	case STATE_OUTPUT_AUDIO:
		...
		if(dwPhoneme != pKsd_t->dwLastPhoneme) {
			...
			// not frames.
			dwDuration =
			    (1000 * dwDuration * pVtm_t->uiNumberOfSamplesPerFrame) / pKsd_t->uiSampleRate;

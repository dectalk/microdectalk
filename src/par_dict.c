...
char *d_lookup(unsigned char *llp, char *table) {
    // bytes 0-3
    int entries = get_long_int((char *)table);
    // bytes 4-7
    ...
    for (int j = 0; j < entries; j++) {
        ...
        for (i = 0; entry[i] != 0; i++) {
            ...
            if (entry[i] != par_lower[llp[i]]) { // lower
                test = 0;
                break;
            }
	}
        if ( par_lower[llp[i]] != 0 ) { // lower
            continue;
        }
        if (!test) {
            continue;
        ...
        if (par_lower[llp[0]] == 't') fc_struct[fc_index] = FC_TO | FC_PREP | FC_FUNC;
        if (par_lower[llp[0]] == 'a') fc_struct[fc_index] = FC_CONJ | FC_VERB | FC_FUNC;
        if (par_lower[llp[0]] == 'f') fc_struct[fc_index] = FC_ADV | FC_PREP | FC_NEG;
        #endif
...
int _far par_dict_lookup(LPTTS_HANDLE_T phTTS, unsigned char *word, int in_flag) {
    char *cp;
    //unsigned char *DICT_DATA = pKsd_t->fdic_data[pKsd_t->lang_curr];

    if (main_dict && (cp = (char *)d_lookup(word, &main_dict[0])) != NULL) {
        printf("Hit: %s\n", word);
        //sendlist(cp);
        ls_util_send_asky_phone_list(phTTS, cp);
        //lbphone=WBOUND;
        //rbphone=WBOUND;
        return (HIT);
    }
                   ...
                   long index,struct dic_entry far **ppent,unsigned char *word)
{
	...
	if (index>limit)
	{
        ...
        if (index >= fdic_entries-1) {
            return LOOK_LOWER; // prevent overflow
        }

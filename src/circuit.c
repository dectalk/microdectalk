...

{
	...
	*****/
	state->agx =
	    frame->ag + CMSQ_TO_MMSQ(state->Pm * (state->Cg < 0.0f ? 0.0f : state->Cg) * speaker->Lg);
	...
	/*****
	  Flows are in CGS
	*****/
	state->Ug = (float)(FLOW_SQRT(2.0f * (CMWATER_TO_CGS(frame->ps) - state->Pm) / RHO) * MMSQ_TO_CMSQ(state->agf));
...
PmRootFunction(float Pm, void* pVoidOtherArgs) {
	...
	Uw   = (float)(-(pOtherArgs->ue) +
		       pOtherArgs->rootTwoOverRho *
			   (agf * FLOW_SQRT(pOtherArgs->ps - Pm) - (pOtherArgs->acx + pOtherArgs->an) * FLOW_SQRT(Pm)));

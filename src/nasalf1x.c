...

{
	...
	*pFNZ = fn * (float)DTsqrt((1.f + MmOverMn) / (1.f + MmOverMn * fn * fn / (fm * fm)));

#ifdef DEBUG_OLD
	if(!((fn > *pFNZ && fm < *pFNZ) || (fn < *pFNZ && fm > *pFNZ))) {
...

{
	...
	*****/
	...
	if(frame->f1 >= speaker->fm_f1BreakPoint)
		...
		fm = (float)(r * (0.8f * frame->f2 + 0.2f * frame->f3) + (1. - r) * 3000.);
	}
...

{
	...
	*****/
	...
	temp = speaker->B1m + MMSQ_TO_CMSQ(an) * NasalBandwidth;
	...
	*****/
	...
	if(state->f1c >= speaker->fno) {
		...
		if(an < speaker->anaTable[0].Column1) { /* interpolate from the first tabulated value of a to
		...
		if(an < speaker->anbTable[0].Column1) { /* linearly interpolate from the first tabulated value
							   of b to b = 0 at an = 0 */
...

{
	...
	*****/
	...
	FNPvars.fn = InterpolateTable(anfnTable, MAXANFN, an) * (speaker->fno / anfnTable_fno);

	if(frame->f2 >= speaker->fp_f2BreakPoint)
		FNPvars.fp = (float)(0.00036f * state->f1c * (frame->f2 - speaker->fp_f2BreakPoint) + (speaker->fp_f2BreakPoint - 100.f));
	else
...

{
	...
	if(FNPvars->fn >= f1c) {
		*Brac_low = (float)(FNPvars->fn + FINITE_OFFSET * (FNPvars->fp - FNPvars->fn));
		min_f	  = FNPvars->fn;
	} else
		min_f = *Brac_low = f1c;

	*Brac_high = (float)(FNPvars->fn + (1.0f - FINITE_OFFSET) * (FNPvars->fp - FNPvars->fn));

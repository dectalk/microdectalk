...
cm_cmd_mtone(LPTTS_HANDLE_T phTTS) {
	...
	if((pCmd_t->defaults[0] == true) || (pCmd_t->defaults[1] == true) || (pCmd_t->defaults[2] == true) || (pCmd_t->defaults[3] == true)) {
...
cmd_mod_tone(int freq, int on_time, int off_time, int cycles) {
	...
	while((!KS.halting) && ((cycles == 0) || (cycle_count < cycles))) {
		unsigned int i;
		cycle_count++;
		if((cycle_count > 2) && (cycles == 0)) {

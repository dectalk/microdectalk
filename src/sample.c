...
Float next_sample(LLSynth* synth, LLFrame* frame) {
	...
	noise = (short)(synth->state.random = ((synth->state.random * 20077L + 12345L) % 65536L)) / 65536.f;
	...
	if(!synth->state.parallel_only_flag) {
		...
		case 8:
			casc = AdvanceResonator(&synth->formant_8_cascade, casc);
		case 7:
			casc = AdvanceResonator(&synth->formant_7_cascade, casc);
		case 6:
			casc = AdvanceResonator(&synth->formant_6_cascade, casc);
		case 5:

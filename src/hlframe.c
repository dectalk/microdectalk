...
{
	if(frame->f0 > 0.0f) {
		llframe->NF0 = (short)(frame->f0 + 0.5f /* round, not truncate */
#ifdef in_phdraw					// help help help the
				       /* Correction term for vowel height */
				       + (frame->f1 <= speaker->f0_vowelshift_f1_break ? speaker->Kf1 * frame->f0 * (speaker->f1_neutral - speaker->f0_vowelshift_f1_break) : (frame->f1 < speaker->f1_neutral ? speaker->Kf1 * frame->f0 * (speaker->f1_neutral - frame->f1) : 0.0f))
...

{
	...
	****/
...
#endif
	{
...
#ifdef INVTMIONT
			if(frame->f2 > ALV_F2_MAX || frame->f3 > ALV_F3_MAX || frame->f2 < ALV_F2_MIN || frame->f3 < ALV_F3_MIN) {
				llframe->A2f = 0;
				llframe->A3f = 0;
				llframe->A4f = 0;
				llframe->A5f = 0;
		...
		{
			if(frame->place == 40) {
				if(frame->f2 > (speaker->PalVelar_f2Offset + speaker->PalVelar_f2Overf3_Slope * frame->f3)) {
...

{
	...
	*****/
	...
	if(state->agx < speaker->agMin || state->agx > speaker->agAVModalOffsetMax + speaker->agm || frame->ag >= speaker->agHiKLSourceCutoff || frame->ps - CGS_TO_CMWATER(state->Pm) < FLOAT_EPS || frame->ps - CGS_TO_CMWATER(state->Pm) < speaker->AVPressureThreshold - speaker->KdPTdc * frame->dc)
		llframe->NAV = 0;

	else if(state->agx < speaker->agm)
		llframe->NAV = (short)(30. * DTlog10(frame->ps - CGS_TO_CMWATER(state->Pm)) + speaker->Kv - speaker->KdAV0 * MMSQ_TO_CMSQ(speaker->agm - state->agx));

	else if(state->agx < speaker->agm + speaker->agAVModalOffsetOnOff)
		llframe->NAV = (short)(30. * DTlog10(frame->ps - CGS_TO_CMWATER(state->Pm)) + speaker->Kv - speaker->KdAV * MMSQ_TO_CMSQ(state->agx - speaker->agm));

	else

		llframe->NAV = (short)(30. * DTlog10(frame->ps - CGS_TO_CMWATER(state->Pm)) + speaker->Kv - speaker->KdAV * MMSQ_TO_CMSQ(speaker->agAVModalOffsetOnOff) - speaker->KdAV1 * MMSQ_TO_CMSQ(state->agx - speaker->agm - speaker->agAVModalOffsetOnOff));
	...
	*****/
	...
	if(state->agf < speaker->agMin || frame->ag >= speaker->agHiKLSourceCutoff || (frame->an <= 0. && state->acx <= 0.) || fabs(frame->ps - CGS_TO_CMWATER(state->Pm)) < FLOAT_EPS || state->agf < FLOAT_EPS)
		llframe->NAH = 0;

	else

	{
		llframe->NAH = (short)(30.f * DTlog10((float)fabs(frame->ps - CGS_TO_CMWATER(state->Pm))) + 10.0f * DTlog10(MMSQ_TO_CMSQ(state->agf)) + speaker->Ka);
	}
	/*****
	  Must not be negative. From Williams.
	*****/
...

{
	...
	*****/
	...
	NumberAFInterpolations = (int)(speaker->UpdateInterval / speaker->AFInterpTimeStep);

	acxPrev = MMSQ_TO_CMSQ(oldstate->acx);
	acxStep = (MMSQ_TO_CMSQ(state->acx) - acxPrev) / (float)NumberAFInterpolations;

	PmPrev = CGS_TO_CMWATER(oldstate->Pm);
	PmStep = (CGS_TO_CMWATER(state->Pm) - PmPrev) / (float)NumberAFInterpolations;
...

{
	...
	if(state->agf > speaker->agm) {
		llframe->NB3 = (short)(speaker->B3m + (state->agf - speaker->agm) * speaker->KB3);
		llframe->NB4 = (short)(speaker->B4m + (state->agf - speaker->agm) * speaker->KB4);
		llframe->NB5 = (short)(speaker->B5m + (state->agf - speaker->agm) * speaker->KB5);
	}
	...
	if(state->agf > speaker->agm) {
		...
		llframe->NB1 = (short)(state->b1x + a * MMSQ_TO_CMSQ(state->agf - speaker->agm) * DTsqrt(Ptransg) / (Ptransg + b * state->f1x * state->f1x));
		llframe->NB2 = (short)(speaker->B2m + a * MMSQ_TO_CMSQ(state->agf - speaker->agm) * DTsqrt(Ptransg) / (Ptransg + b * frame->f2 * frame->f2));
	}
...
SourceSpecifics(HLFrame* frame, HLSpeaker* speaker, HLState* state,
	...
	/*****
	  Compute OQ
	*****/
	llframe->NOQ = (short)(speaker->OQm + (state->agx - speaker->agm) * speaker->KOQ);
	...
	*****/
	...
	if(acx_anMax < speaker->TLBreakArea) {
		TLFloat = speaker->TLm + ((speaker->TLBreakArea - acx_anMax) + (state->agx - speaker->agm)) * speaker->KTL;
	} else
	...
	*****/
	...
	m = fabs(speaker->At) > L_EPS * L_EPS ? speaker->Lt / speaker->At : (
#ifdef WARNINGS
										printf("SourceSpecifics:  used default area of trachea."),
#endif
										speaker->Lt / 2.5f);

	m += fabs(speaker->Av) > L_EPS ? speaker->Lv / speaker->Av : (
#ifdef WARNINGS
	...
	SixkHzpiT	 = 6000.0f * (float)PI * apCgs2 * (m * apCgs + speaker->Lvg) / (RkApCubedOverRho + RvApCubedOverRho);
	...
	*****/
	...
	llframe->NTL = (short)(TLFloat + (speaker->SFromf4 * frame->f4 - speaker->SDefault) * speaker->dBTLforPctS / (speaker->PctSfordBTL * speaker->SDefault));

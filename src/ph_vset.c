...
void setparam(LPTTS_HANDLE_T phTTS, int which, int value) {
	...
	if(pVtm_t->bDoTuning) /* Ignore autotuned vals when autotuning */ //
	{
		...
		if(value < lp->l_min) { /* Lower limit check.   */
			value = lp->l_min;
		} else if(value > lp->l_max) { /* Upper limit check.   */
			value = lp->l_max;
		}
...
void usevoice(LPTTS_HANDLE_T phTTS, int voice) {
	...
	for(i = 0; i < SPDEF; ++i) {
		...
		value = 0;
		lp    = &limit[i];
		if(pDph_t->curspdef[i] < lp->l_min) { /* Lower limit check.   */
			value = lp->l_min;
		} else if(pDph_t->curspdef[i] > lp->l_max) { /* Upper limit check.   */
			value = lp->l_max;
		}

		...
		if(np->tspesh > 0) {
			if(pDph_t->tcum < np->tspesh) {
				...
				*parp = np->pspesh;
				...
				if((np > &PAP) && (pDph_t->tcum == (np->tspesh + 1)) && (*parp >= 10)) {
...
#ifdef HLSYN
				if(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FSON1 && !(pDph_t->allofeats[pDph_t->nphone] & FDUMMY_VOWEL)) {
					if(pKsd_t->lang_curr == LANG_french) {
					}

					if(pDph_t->tcum == 3 && np->tspesh > 5) {
...
/* Spdef par FT -> 0 < f0_dep_tilt < 100, temptilt = 1/40 (max) of 1400-f0 */
	...
	*parp += (pDph_t->spdeftltoff - 6); /*from fphdraw.c*/
	...
	if(pDph_t->breathysw == 1) {		      /* Set in ph_setar.c */
		if(pDph_t->parstochip[OUT_AV] > 40) { /* Do only if AV on  */
	...
	for(; np <= &PTONGUEBODY; np++) {
		...
		if(np == &PAREAL) {
			if(np->tspesh) {
				...
				} else if(pDph_t->tcum >= (np->tspesh - (bplos_build_time)) && (phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FBURST)) {
			...
			if(np->tspesh) {
				...
				if(pDph_t->tcum >= np->tspesh) {
					...
					if(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FCONSON) {
						...
						pDph_t->in_brelease = 1;
				} else if(pDph_t->tcum >= (np->tspesh - (bplos_build_time + 1)) && (phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FBURST)) {
		...
		if(np == &PTONGUEBODY) {
			...
			if(np->tspesh) {
				...
				if(pDph_t->tcum >= (np->tspesh) && !(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone + 1]) & FSTOP)) {
				...
				else if(pDph_t->tcum == 0 && (phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FSTOP)) {
	...
	if(pDph_t->nphone == 0) // in intial silence anticipate intial parameters
	{
		...
		if(pDph_t->nphone != pDph_t->nphonelast) {
...

			{
				...
				if(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone + 1]) & FBURST && phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone + 1]) & FSTOP)
		...
		if(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone + 1]) & FVOICD) {
			...
			if((phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone + 1]) & FNASAL)) {
				...
				if(pDph_t->allophons[pDph_t->nphone + 1] == USP_M || pDph_t->allophons[pDph_t->nphone + 1] == USP_N)
				...
				if(pDph_t->allophons[pDph_t->nphone + 1] == UK_N || pDph_t->allophons[pDph_t->nphone + 1] == UK_NX || pDph_t->allophons[pDph_t->nphone + 1] == UK_N || pDph_t->allophons[pDph_t->nphone + 1] == UK_NX) {
					pDph_t->area_b	 = 0;
					pDph_t->target_b = 0;
#ifdef PH_DEBUG_OLD
					if(DT_DBG(PH_DBG, 0x080))
		...
		if(pDph_t->allophons[pDph_t->nphone + 1] == USP_DH || pDph_t->allophons[pDph_t->nphone + 1] == USP_TH || pDph_t->allophons[pDph_t->nphone + 1] == USP_DZ || pDph_t->allophons[pDph_t->nphone + 1] == UKP_DH || pDph_t->allophons[pDph_t->nphone + 1] == UKP_TH || pDph_t->allophons[pDph_t->nphone + 1] == UKP_DZ)
	...
	} else // in a regular phoneme
	{
		...
		if(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FOBST && pDph_t->dcstep == 0 && pDph_t->area_n == 0 && pDph_t->phonestep >= 1) {
		...
		if(pDph_t->dcstep != 0) {
			...
			if(phone_feature(pDph_t, pDph_t->allophons[(pDph_t->nphone)]) & FVOICD) {
				if(phone_feature(pDph_t, pDph_t->allophons[(pDph_t->nphone)]) & FOBST) {
					if(pDph_t->dcstep > 0 && pDph_t->tcum >= pDph_t->allodurs[pDph_t->nphone] - 4) {
						pDph_t->dcstep--;
						if(pDph_t->dcstep != 0)
							pDph_t->dcstep--;
#ifdef PH_DEBUG_OLD
				...
				else if(phone_feature(pDph_t, pDph_t->allophons[(pDph_t->nphone)]) & FOBST && pDph_t->area_n == 0) {
				...
				//	pDph_t->parstochip[OUT_DC] = -dcval[-pDph_t->dcstep];
				//	pDph_t->parstochip[OUT_UE] = -ueval[-pDph_t->dcstep];
				if(phone_feature(pDph_t, pDph_t->allophons[(pDph_t->nphone)]) & FOBST && pDph_t->area_n == 0) {
					// if dcstep is of opposite sign it means we're obstr-obstru so chnage faste
					if(pDph_t->dcstep > 0) {
						pDph_t->dcstep--;
						// for v5.
		...
		if((pDph_t->allofeats[pDph_t->nphone] & FSTRESS) == FEMPHASIS
		   /*&& (((phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone])) & FSYLL) IS_PLUS)*/) {
...
#ifdef outfor_now
		else if(!(pDph_t->allofeats[pDph_t->nphone] & FSTRESS) && ((phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]))&FSYLL)) {
			...
			if(place(pDph_t->allophons[pDph_t->nphone + 1]) & FLABIAL && ((phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]))&FPLOSV)) {
		...
		if(pDph_t->nphone != pDph_t->nphonelast) {
			...
			if(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FOBST) {
				...
				if((phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FVOICD)) {
					...
					if(!(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone + 1]) & FOBST) && phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone + 1]) & FVOICD) {
...
#ifdef evaluate
					if(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FSTOP && !(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone] - 1) & FVOICD) && !(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FVOICD)) {
#ifdef PH_DEBUG_OLD
						if(DT_DBG(PH_DBG, 0x080)) {
			...
			if(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FSTOP && !(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FBURST)) {
			...
			// THIS IS FOR HAND TUNE CODE WHERE PHONEMES ARE DUPLICATED FOR PITCH REASONS
			{
				if(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FSON1) {
					...
					else if(!((phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone - 1]) & FPLOSV) && !(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone - 1]) & FVOICD))) {
						pDph_t->target_ag = pVtm_t->NOM_VOIC_GLOT_AREA;
	...
	if(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FVOWEL) {
		...
		// helpme  help me too: added the diphthongs.
		{
			...
			if((pDph_t->tcum >= (pDph_t->allodurs[pDph_t->nphone] - 4) && pDph_t->tcum > 8) && (!(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone + 1]) & FVOICD))) {
#ifdef PH_DEBUG_OLD
				if(DT_DBG(PH_DBG, 0x080)) {
		...
		if(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone + 1]) & FOBST)
			if(!(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone + 1]) & FSTOP) && !(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone + 1]) & FBURST)) {
				...
				if(pDph_t->tcum >= ((pDph_t->allodurs[pDph_t->nphone] - 1))) {
					...
					if(place(pDph_t->allophons[pDph_t->nphone]) & FDENTAL || place(pDph_t->allophons[pDph_t->nphone + 1]) & FLABIAL) {
...
#ifndef TOMBUCHLER
	if(!(pDph_t->allofeats[pDph_t->nphone + 1] & FDUMMY_VOWEL) && !(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone + 1]) & FVOICD) && phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone + 1]) & FSONOR && phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone + 1]) & FCONSON) {
...
#endif
	if(!(pDph_t->allofeats[pDph_t->nphone] & FDUMMY_VOWEL) && !(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FVOICD) && phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FSONOR && phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FCONSON) {
		pDph_t->target_l  = 1000;
		pDph_t->target_b  = 1000;
		pDph_t->target_tb = 1000;
	...
	if(!(pDph_t->allofeats[pDph_t->nphone + 1] & FDUMMY_VOWEL) && !(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone + 1]) & FVOICD) && phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone + 1]) & FSONOR && phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone + 1]) & FCONSON && (pDph_t->tcum >= (pDph_t->allodurs[pDph_t->nphone] - 7))) {
#ifdef PH_DEBUG_OLD
		if(DT_DBG(PH_DBG, 0x080)) {
	...
	if(!(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FNASAL)) {
		if(place(pDph_t->allophons[pDph_t->nphone]) & FDENTAL) {
			...
			if(pDph_t->allophons[pDph_t->nphone] == USP_DH || pDph_t->allophons[pDph_t->nphone] == USP_TH || pDph_t->allophons[pDph_t->nphone] == USP_DZ || pDph_t->allophons[pDph_t->nphone] == UKP_DH || pDph_t->allophons[pDph_t->nphone] == UKP_TH || pDph_t->allophons[pDph_t->nphone] == UKP_DZ) {
			...
			// blade not affect so set it open
			if((pDph_t->allophons[pDph_t->nphone] == USP_JH || pDph_t->allophons[pDph_t->nphone] == USP_CH || (pDph_t->allophons[pDph_t->nphone] == GRP_SH && pDph_t->allophons[pDph_t->nphone - 1] == GRP_T))) {
		...
		// the new place hence
		if((place(pDph_t->allophons[pDph_t->nphone - 1]) & FPALATL || place(pDph_t->allophons[pDph_t->nphone - 1]) & FDENTAL || place(pDph_t->allophons[pDph_t->nphone - 1]) & FALVEL || place(pDph_t->allophons[pDph_t->nphone - 1]) & FLABIAL) &&
		   !(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone - 1]) & FSTOP && phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FOBST) &&
		   // eab if followed by a fricative the release ends up coarticulated to
	...
	if(pDph_t->nphone)
		...
		if(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FSONCON) {
			...
			if(pDph_t->tcum <= (pDph_t->allodurs[pDph_t->nphone] >> 1)) {
				if(pDph_t->target_ag < pVtm_t->NOM_VOIC_GLOT_AREA + 300 && (pDph_t->allophons[pDph_t->nphone] != USP_R))
			...
			if(pDph_t->allophons[pDph_t->nphone] == FP_R || pDph_t->allophons[pDph_t->nphone] == USP_W || pDph_t->allophons[pDph_t->nphone] == USP_R)
	...
	if((pDph_t->allophons[pDph_t->nphone + 1] == USP_LL || pDph_t->allophons[pDph_t->nphone + 1] == USP_LX) && pDph_t->tcum >= (pDph_t->allodurs[pDph_t->nphone] - 4) && (!phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FSTOP)) {

		if(pDph_t->target_ag > 600)
			pDph_t->target_ag = 600;
	}

	if(pDph_t->allophons[pDph_t->nphone] == USP_LL || pDph_t->allophons[pDph_t->nphone] == USP_LX) {
		pDph_t->parstochip[OUT_A2] = 4000;
		pDph_t->target_ag	   = 600;
		pDph_t->agspeed		   = 3;
	}
	...
	// Nasal velum rules
	if(place(pDph_t->allophons[pDph_t->nphone]) & FVELAR && phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FNASAL)
	// ng so set tbclosure
	{
	...
	if(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FNASAL) {
		...
		if(place(pDph_t->allophons[pDph_t->nphone]) & BLADEAFFECTED && place(pDph_t->allophons[pDph_t->nphone - 1]) & BLADEAFFECTED) {
		...
		else if((phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone + 1]) & FOBST) && pDph_t->tcum >= (pDph_t->allodurs[pDph_t->nphone] - 1) && !(pDph_t->allophons[pDph_t->nphone + 1] == USP_DH))
		...
		if(pDph_t->allodurs[pDph_t->nphone] <= 10) {
			if(pDph_t->tcum < 4 && pDph_t->nasal_step < 1) {
				...
				if(pDph_t->nasal_step < 7 && pDph_t->nasal_step) {
					pDph_t->nasal_step++;
				}
			}
		}
		...
		&& pKsd_t->lang_curr != LANG_french && pDph_t->allophons[pDph_t->nphone + 1] != pDph_t->allophons[pDph_t->nphone + 2]) {
		...
		{
			if(!(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FOBST) && pDph_t->tcum >= (pDph_t->allodurs[pDph_t->nphone] - 2))
	...
	// we were in a nasal at some previous point
		...
		skipit:
			...
			if(!(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FOBST) && phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone + 1]) & FOBST && !(pDph_t->allofeats[pDph_t->nphone] & FSTRESS) && !(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone + 1]) & FVOICD) && !(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone + 1]) & FSTOP)) {
				...
				if(pDph_t->tcum >= (pDph_t->allodurs[pDph_t->nphone] - NF60MS)
				   ...
				   && pDph_t->tcum >= pDph_t->allodurs[pDph_t->nphone] >> 1) {
					...
					if(pDph_t->target_ag < 700)
						pDph_t->target_ag += 75;
					else if(pDph_t->target_ag < 1100)
						pDph_t->target_ag += 40;
				}
			}
			if(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FOBST && (phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FPLOSV) && (phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone + 1]) & FNASAL))
			...
			else if(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FOBST && !(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FPLOSV) && phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone + 1]) & FVOICD && !(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone + 1]) & FNASAL))

			{
				...
				if(pDph_t->target_ag >= 800) {
					...
					if(pDph_t->tcum >= (pDph_t->allodurs[pDph_t->nphone] - NF50MS) && !(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone + 1]) & FSTOP)) {
#ifdef PH_DEBUG_OLD
						if(DT_DBG(PH_DBG, 0x080)) {
	...
	if(pDph_t->target_tb < pDph_t->area_tb) {
		...
		if(pDph_t->in_tbrelease == 1 && pDph_t->area_tb < 1000) {
			...
			if(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone - 1]) & FBURST ||
			   phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FBURST) { // plosive release sits stays at fric value until one step after bstep =0
			...
			if(place(pDph_t->allophons[pDph_t->nphone]) & BLADEAFFECTED && place(pDph_t->allophons[pDph_t->nphone - 1]) & FVELAR) {
	...
	if(pDph_t->in_tbclosure) {
		if(place(pDph_t->allophons[pDph_t->nphone]) & FVELAR) {
			if(!(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FSTOP) && pDph_t->allophons[pDph_t->nphone] != GEN_SIL) {
...
#endif
				if(pDph_t->phonestep > 0 || pDph_t->allophons[pDph_t->nphone] == GEN_SIL) {
					pDph_t->target_b = 1000;
					pDph_t->target_l = 1000;
	...
	if(pDph_t->in_tbrelease || (pDph_t->tstep && pDph_t->tstep < 7)) {
		...
		if(pDph_t->tstep <= 3) {
			if(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FSTOP && pDph_t->tstep == 1
	...
	if(place(pDph_t->allophons[pDph_t->nphone - 1]) & FVELAR && phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone - 1]) & FNASAL && !(place(pDph_t->allophons[pDph_t->nphone]) & FVELAR))
		...
		if(pDph_t->phonestep <= 2 || pDph_t->allophons[pDph_t->nphone] == GEN_SIL) {
	...
	if(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FSTOP && place(pDph_t->allophons[pDph_t->nphone]) & FVELAR && phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone + 1]) & FSTOP) {
	...
	if(pDph_t->allophons[pDph_t->nphone] == FP_V && pDph_t->tcum > (pDph_t->allodurs[pDph_t->nphone] - 4)) {
		pDph_t->target_l += 30;
	}
	...
	else if(pDph_t->target_b != pDph_t->area_b || pDph_t->bstep == -1) // HELPME MGS
	{
		...
		if(pDph_t->in_brelease == 1 && pDph_t->area_b < 1000) {
...
#endif
				    if(pDph_t->target_b <= 200 && pDph_t->area_b < 100)
					pDph_t->area_b = pDph_t->target_b;
				else
					pDph_t->area_b += ((pDph_t->target_b - pDph_t->area_b) >> 1);
			...
			// if it's a stop
			if(place(pDph_t->allophons[pDph_t->nphone]) & FLABIAL || place(pDph_t->allophons[pDph_t->nphone]) & FVELAR) {
				if(pDph_t->phonestep < coarticulation) {
					pDph_t->area_b = pDph_t->last_area_b;
					// and if previous was a blade fric close it briefly
					if(place(pDph_t->allophons[pDph_t->nphone - 1]) & BLADEAFFECTED && pDph_t->area_b < 200)
						pDph_t->area_b = 0;
				}
#ifdef PH_DEBUG_OLD
				if(DT_DBG(PH_DBG, 0x080)) {
	...
	if(pDph_t->allophons[pDph_t->nphone] == USP_DH || pDph_t->allophons[pDph_t->nphone] == USP_TH || pDph_t->allophons[pDph_t->nphone] == UKP_DH || pDph_t->allophons[pDph_t->nphone] == UKP_TH)
	...
	if(pDph_t->had_in_phrase_final) {
		if(pDphsettar->nframb > (pDph_t->tcumdur - 92)) {
			...
			if(pDph_t->nphone >= (pDph_t->nallotot - 4) && pDph_t->nphone < pDph_t->nallotot - 1) {
...
#endif
			if(pDph_t->nphone >= (pDph_t->last_real_phon - 2) && phone_feature(pDph_t, pDph_t->allophons[(pDph_t->nphone)]) & FVOICD)
...
// #define OLD_PRESSURE
#ifdef OLD_PRESSURE
#ifdef notneedednow
	if((pDph_t->allofeats[pDph_t->nphone + 1] & FDUMMY_VOWEL) || (pDph_t->allofeats[pDph_t->nphone] & FDUMMY_VOWEL))
	...
	if(pDph_t->allophons[pDph_t->nphone] != GEN_SIL) {
		if(pDphsettar->nframb > (pDph_t->tcumdur - 10) && pDph_t->tcumdur > 60) {

			if(pDphsettar->diplo <= 480) {
				pDphsettar->diplo += 38;
			}

		} else if(pDphsettar->nframb > (pDph_t->tcumdur - 15) && pDph_t->tcumdur > 120) {
	...
	if(pDphsettar->nframb > (pDph_t->tcumdur - 15) && (pDph_t->tcumdur > 15))
		...
		if(pDph_t->nphone == pDph_t->nallotot - 1) {
			if(pDph_t->tcum >= pDph_t->allodurs[pDph_t->nphone] - 50 /*NF80MS*/) {
				if(phone_feature(pDph_t, pDph_t->allophons[(pDph_t->nphone)]) & FOBST) {
					...
					if(pDph_t->pressure_drop < 3000 && !(phone_feature(pDph_t, pDph_t->allophons[(pDph_t->nphone)]) & FVOICD)) {
    ...
    if(((pDph_t->allofeats[pDph_t->nphone] & FBOUNDARY) >= FPPNEXT || (pDph_t->allofeats[pDph_t->nphone + 2] & FBOUNDARY) >= FPPNEXT || (pDph_t->allofeats[pDph_t->nphone + 3] & FBOUNDARY) >= FPPNEXT) && pDph_t->syl_pressure == 0) {
	...
	if((pDph_t->nphonetot - pDph_t->nphone) <= 4 && pDph_t->syl_pressure == 0) {
		...
		if(pDph_t->syl_pressure == 0 && pDphsettar->nframb > 9 && (value - pDphsettar->nframb) > 0) {
			if(pKsd_t->lang_curr == LANG_german) {
...
if(pDphsettar->nframb <= 1) {
	...
	// TEST
	if(!(pDph_t->allofeats[pDph_t->nphone] & FSTRESS) && (phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FSYLL)) {
	...
	if(((pDph_t->target_ag - pDph_t->area_g) >> pDph_t->agspeed) > 300)
		...
		pDph_t->area_g += ((pDph_t->target_ag - pDph_t->area_g) >> pDph_t->agspeed) + pDph_t->delta_area_g; //+ pDph_t->delta_a_forap;
														    // for high speed
	...
	if(pDph_t->allophons[pDph_t->nphone] == GRP_KH || pDph_t->allophons[pDph_t->nphone] == SPP_J || pDph_t->allophons[pDph_t->nphone] == LAP_J)
	...
	{
		if(pDph_t->allophons[pDph_t->nphone + 1] == LAP_Y || pDph_t->allophons[pDph_t->nphone + 1] == SPP_Y) {
			pDph_t->parstochip[OUT_PLACE] = 45;
		} else {
...
tmp -= pDph_t->pressure_gest;
#endif
	if(tmp < 100 && pDph_t->nphone > 1 || tmp < 0) {
		pDph_t->pressure_drop = 0;
		pDph_t->pressure      = 0;
		pDph_t->syl_pressure  = 0;
		tmp		      = 0;
	...
	if((pDph_t->allophons[pDph_t->nphone + 1] == LAP_R || pDph_t->allophons[pDph_t->nphone + 1] == SPP_R) && pDph_t->allophons[pDph_t->nphone] == LAP_G) {
		pDph_t->target_ag = 700;
	}

	if(pDph_t->allophons[pDph_t->nphone] == LAP_R || pDph_t->allophons[pDph_t->nphone] == SPP_R) {
	...
	else if(pDph_t->allophons[pDph_t->nphone] == SPP_RR || pDph_t->allophons[pDph_t->nphone] == LAP_RR) {
		...
		/* Initialiatize the modulation counter eab 2/6/98*/
		if(pDphsettar->phcur != USP_CZ && pDphsettar->phcur != UKP_NX)
	...
	if((pDph_t->allophons[pDph_t->nphone + 1] == GEN_SIL) && (pDph_t->allophons[pDph_t->nphone] == GRP_EX))
	...
	if((pDph_t->allophons[pDph_t->nphone + 1] == GEN_SIL) && (pDph_t->allophons[pDph_t->nphone] == SPP_IX))
		pDph_t->parstochip[OUT_AV] -= 3;

	if((pDph_t->allophons[pDph_t->nphone + 1] == GEN_SIL) && (pDph_t->allophons[pDph_t->nphone] == LAP_IX))
		pDph_t->parstochip[OUT_AV] -= 3;
	...
	if(pDph_t->allophons[pDph_t->nphone] == UKP_LL || pDph_t->allophons[pDph_t->nphone] == USP_LL || pDph_t->allophons[pDph_t->nphone] == FP_L || pDph_t->allophons[pDph_t->nphone] == SPP_L || pDph_t->allophons[pDph_t->nphone] == LAP_L || pDph_t->allophons[pDph_t->nphone] == GRP_L)

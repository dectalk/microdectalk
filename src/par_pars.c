...
void par_match_rule(unsigned char*  current_rule,
	...
	if(current_rule == NULL || input_array == NULL || output_array == NULL || match_array == NULL) { /* return a failed value */
...
#if defined(DEBUG_OLD) || defined(INDEX_DEBUG_OLD)
...
static int dt_stricmp(const char* a, const char* b) {
	while(*a && *b) {
...
short par_search_for_word(unsigned char* input,
...
#endif
		if((value = dt_stricmp(input, (unsigned char*)(dict_data_table + dict_index_table[pos]))) == 0) {
			break;
		} else {
			if(value < 0) {
				for_same = pos - 1;
	...
	rev_same = pos - 1;
	while((rev_same >= low) && (dt_stricmp(input, (unsigned char*)(dict_data_table + dict_index_table[rev_same])) == 0)) {
		rev_same--;
	}
	for_same = pos + 1;
	while((for_same >= low) && (dt_stricmp(input, (unsigned char*)(dict_data_table + dict_index_table[for_same])) == 0)) {
		for_same++;
	}
	rev_same++;
	for_same--;
	...
	while((current_rule[rule_p] != '>') && (end_of_type == 0)) {
		while((current_rule[rule_p] != '>') && (match_is_over == 0) && (par_look_ahead_success == 0) && (end_of_type == 0)) {
			...
			case '*':
				...
				max_range = INT_MAX;
				rule_p++;
				if(current_rule[rule_p] != '>') { /* once a star is found, there is no point in having any more more in the range */
					if(lookahead) {
			...
			case '+':
				...
				max_range = INT_MAX;
				rule_p++;
				if(current_rule[rule_p] != '>') { /* once a plus is found, there is no point in having any more more in the range */
					if(lookahead) {

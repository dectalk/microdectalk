...
/*unsigned int c;*/
{
	...
	if(KS.text_flush || (KS.cmd_flush == CMD_flush_toss))
		{
		isArabicDigit=1; /*flushing so reiinit everything*/
		sndtel=0;
		digcnt=0;
		postel=0;
		last_punct=0;
...
/*                                      if(prcnt >= 10)
					...
					if( last_char == '-' && last_punct == '-' && isArabicDigit == 2 && (KS.modeflag & MODE_MATH) == 0 )
						{
#ifdef DEBUGPARS
		...
		printf("                        c= %c last_char = %c %d isdi= %d\n",c,last_char,last_char,isArabicDigit);
#endif
				
				if( !(KS.sayflag & SAY_LETTER) )
					{
					if(cbufcnt != 0 && isArabicDigit == 1)
						{
...
/* If anything shows up that makes it not a digit then clear flag.*/                                    
					if(( isArabicDigit == 1 && ((char_types[last_char] & (MARK_digit | MARK_space | MARK_clause)) == 0) ||
	...
	if (posord == 0)
		...
		printf("CMD_PARS: process_char: writing heldchar 1 %c \n",heldchar[1]);
			...
			/* If we get a digit in a text string break it out with a space*/
				...
				if( isArabicDigit == 2 && (nchar_types[c] & MARK_digit)
					&& (c != ')'&& c !='(' ) > 0
...
/**/
				...
				if(!(KS.sayflag & SAY_LETTER))
					{
					if (lpchar == '.' || cbufcnt >= 115 || isArabicDigit == 1 ||
		...
		printf("CMD_PARS: process_char: isdig %d  char-t %d\n",isArabicDigit,char_types[c]);
						...
						if( ( ( (char_types[c] & MARK_clause) == 0) || c=='-') && !(isArabicDigit == 1 && last_char == ',' && c == ' ') &&  lpchar != '.' )
							{
							if(c==' ' || c=='-')
								{
								if(digcnt == 3)
		...
		printf("CMD_PARS: process_char: breaking \n");
						...
						sendat();
						isArabicDigit=1; /*assume it's a digi unless we find otherwise*/
						sndtel=0;
						digcnt=0;
						postel=0;
						last_punct=0;
...
int offset,k;
		...
		if( ( (char_types[c] & MARK_space) >= 0) &&
			...
			&& ( (pcnt==1 && isArabicDigit != 1 && last_char !=')')/* ||
			(pcnt >1 && isArabicDigit == 1 && (last_char != '.' && pcnt ==2 ) )*/ )
			&& modechng == 0  )
		{
			pipe_value = (PFASCII << PSFONT) + ' ';
#ifdef SINGLE_THREADED

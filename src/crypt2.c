...
void encryptString(const unsigned char* string, unsigned long key, unsigned char* output) {
	...
	tmplong = ((((unsigned long)csum << 8) & 0xffff00)) | ((unsigned long)vpar & 0xff);
	...
	while(!done && (string[inat] != '\0')) { /* generate the new key (step b) */
		key = trand(key);
...
unsigned int decryptString(const unsigned char* input, unsigned long key, unsigned char* string) {
	...
	inat += 4;
	if(!retval) { /* oops.. couldn't read the input ..*/
		return (retval);
	}
 ...
 *
 * *****************************************************************/
unsigned long trand(unsigned long in) { /* pseudorandom numbers.. */
#ifndef ACI_LICENSE

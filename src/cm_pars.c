		...
		*/
		...
		if(!(pKsd_t->sayflag & SAY_LETTER) && !(pKsd_t->sayflag & SAY_FLETTER)) {
			...
			if((*isAdigit == 1 && ((char_types[pCmd_t->last_char] & (MARK_digit | MARK_space | MARK_clause)) == 0) ||
			    (char_types[c] & (MARK_digit | MARK_space | MARK_clause)) == 0) ||
			   (*posord == 1)) {
		...
		if((*isAdigit == 2) &&
		   (nchar_types[c] & MARK_digit) && (c != ')' && c != '(') > 0 && (pKsd_t->modeflag & MODE_MATH) == 0) {
			...
			if((pCmd_t->lpchar == '.') || ((*cbufcnt) >= 115) || (*isAdigit == 1) ||
			   ((pKsd_t->sayflag == SAY_LINE) && (c != CR) && (c != LF))) {
				...
				if(((char_types[c] & MARK_space) == 0) &&
				       (c != '-' && pCmd_t->digcnt != 3) ||
				   ((c == '.') && (*sndtel == 1)) || ((pCmd_t->digcnt >= 1) && ((c == ',') || (c == '.')))) {
#ifdef DEBUG_OLDPARS
		...
		ndone:
			break; /*handle period and dash later by code above*/
		}
		pCmd_t->postel = 0;
		*sndtel	       = 0;
#ifdef DEBUG_OLDPARS
...
void cm_pars_proc_char(LPTTS_HANDLE_T phTTS,
	...
	/* GL 03/25/1997 BATS#314 force SYNC after 0xb */
	if(c == 0xb) {
#if defined(OLD_INTONATION_AND_TIMING) && !defined(HANG_FIXES)

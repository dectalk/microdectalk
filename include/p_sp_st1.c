...
short sp_gettar(LPTTS_HANDLE_T phTTS, int nphone_temp) {
	...
	if ((pDphsettar->par_type = partyp[npar]) IS_FORM_FREQ_OR_BW)
	{
		...
		if((npar == (B3 - F1)) && (phone_temp == SPP_N || phone_temp == SPP_NH || phone_temp == SPP_NX) && (((sp_place[phnex_temp & PVALUE] & F2BACKI) IS_PLUS) || ((sp_place[phlas_temp & PVALUE] & F2BACKI) IS_PLUS))) {
		 ...
		 */
		if((npar == (B3 - F1)) && ((sp_place[phone_temp & PVALUE] & F2BACKI) IS_PLUS) && (phlas_temp == SPP_F)) {
			tartemp = 90;
		}
		/*
		 * Special rule for /r/ and /rr/ after "back" vowel.        8-Jul-86
		 */
		if((phone_temp == SPP_R || phone_temp == SPP_RR) && (phlas_temp == SPP_O || phlas_temp == SPP_U)) {
			switch(npar) {
			case(F1 - F1):
				tartemp -= 100;
				break;
	...
	else if (pDphsettar->par_type IS_PARALLEL_FORM_AMP)
	{
		...
		if(pDphsettar->np == &PTILT) {
			...
			else if((phone_feat & (FVOICD | FPLOSV)) == (FVOICD | FPLOSV) && ((phone_temp != SPP_DH) || (phone_temp == SPP_YH))) {
				tartemp = 12;
			...
			else if((phone_temp >= SPP_DH) && (phone_temp <= SPP_GH)) {
				tartemp = 24;
			}
...

{
	...
	if(pDphsettar->np == &PF1) {
		...
		case SPP_O:
			...
			if(loc == FLABIAL && other != SPP_F)
				return (-50);
#endif
			if(other == SPP_Y || other == SPP_YH)
				return (100);
			break;
		...
		case SPP_U:
			...
			/* OUT to prevent overload                              30-Apr-86       */
			/* Back in again                                        30-Jul-86       */
			if(other == SPP_M || other == SPP_F)
				return (-50);
#endif
			if(loc == FDENTAL || other == SPP_S || loc == FPALATL)
				return (75);
			break;
		}
...
static void sp_forw_smooth_rules(LPTTS_HANDLE_T phTTS, short shrif, short pholas,
	...
	/* Formant frequencies */
	/* Dipthongs first     */
	if(pDphsettar->par_type IS_FORM_FREQ_OR_BW && (feacur & FVOWEL) IS_PLUS && (fealas & FSEMIV) IS_PLUS && (struccur & FBOUNDARY) == 0) {
	...
	} else if(pDphsettar->par_type IS_FORM_FREQ_OR_BW && (feacur & FSEMIV) IS_PLUS && (fealas & FVOWEL) IS_PLUS && (struccur & FBOUNDARY) == 0) {
		pDphsettar->durtran = pDph_t->durfon >> 1;
		pDphsettar->bouval  = (pDphsettar->np->tarlas + pDphsettar->np->tarcur) >> 1;
		...
		if((feacur & FSONOR) IS_PLUS) {
			if((feacur & FSONCON) IS_MINUS) {
				...
				if((fealas & FSONCON) IS_PLUS) {
					...
					if(pDphsettar->phcur != GEN_SIL && (pholas == SPP_L) && (pDphsettar->np == &PF1)) {
...
/* OUT                  xx += ((bouval - np->tarend) >> 1); np->tarend += xx;   (DOESN'T WORK) END OUT */
			...
			if((feacur & FNASAL) IS_PLUS) {
				...
				else if(((pDphsettar->phcur == SPP_N)) && (begtyp(pDphsettar->phonex) == 1)) {
				...
				else if((pDphsettar->np == &PF2) && (pDphsettar->phcur == SPP_M || pDphsettar->phcur == SPP_MX) && ((sp_place[pholas & PVALUE] & F2BACKF) IS_PLUS)) {
	...
	/* Formant bandwidths */
		...
		if(pholas == GEN_SIL && ((feacur & FSONCON) IS_MINUS)) {
			...
			if(((phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone - 2]) & FVOICD) IS_MINUS) && (pDphsettar->np == &PB1)) {
		...
		/* Special rule for [fi] */
		if((pDphsettar->np == &PB3) && ((sp_place[pDphsettar->phcur & PVALUE] & F2BACKI) IS_PLUS) && (pholas == SPP_F)) {
			pDphsettar->durtran = 0;
	...
	/* Default is max of [average,tarcur-12,tarlas-12] */
		...
		if((pDphsettar->bouval < temp) || ((fealas & FPLOSV) IS_PLUS) || (pholas == SPP_YH)) { /* 7-May-86    */
		...
		if(pDphsettar->np == &PAP) {
			if((pDphsettar->phcur == GEN_SIL) || feacur == (FOBST | FCONSON)) {
				/*
				 * [SI], [F], [TH], [S], [SH]
				 */
...
static void sp_back_smooth_rules(LPTTS_HANDLE_T phTTS, short shrib, short feacur,
	...
	/* Formant frequencies */
	if(pDphsettar->par_type IS_FORM_FREQ_OR_BW && (feacur & FVOWEL) IS_PLUS && (feanex & FSEMIV) IS_PLUS && (strucnex & FBOUNDARY) == 0) {
		pDphsettar->durtran = pDph_t->durfon >> 1;
		pDphsettar->bouval  = (pDphsettar->np->tarnex + pDphsettar->np->tarcur) >> 1;
		// debugbackward (" dipthong (vowel) transition",pDphsettar->np);
	} else if(pDphsettar->par_type IS_FORM_FREQ_OR_BW && (feacur & FSEMIV) IS_PLUS && (feanex & FVOWEL) IS_PLUS && (strucnex & FBOUNDARY) == 0) {
		pDphsettar->durtran = pDph_t->durfon >> 1;
		pDphsettar->bouval  = (pDphsettar->np->tarnex + pDphsettar->np->tarcur) >> 1;
...
/* ****************** NOT USED *************************************/
		  ...
		  NOTE:[y] and[yh] are now     obstruents
			...
			if((feacur & FSONCON) IS_MINUS) {
				if((feanex & FSONCON) IS_PLUS) {
					...
					if(pDphsettar->phcur != GEN_SIL && (pDphsettar->phonex == SPP_L) && (pDphsettar->np == &PF1)) {
						pDphsettar->bouval += 80;
					}
				} else {
				}
		...
		if(pDphsettar->phonex == GEN_SIL) {
			...
			if((feacur & FNASAL) IS_PLUS) {
				...
				else if((pDphsettar->np == &PF2) && (pDphsettar->phcur == SPP_M || pDphsettar->phcur == SPP_MX) && ((sp_place[pDphsettar->phonex & PVALUE] & F2BACKI) IS_PLUS)) {
					pDphsettar->bouval -= 150;
				}
#endif
			}
		...
		if(((feacur & FOBST) IS_MINUS) && (begtyp(pDphsettar->phonex) != OBSTRUENT) /* Unless next seg is obst */
		   && (pDphsettar->durtran > 0)) {
			/// pDph_t->arg1 = pDphsettar->durtran;
			// pDph_t->arg2 = shrib;
	...
	/* Source amplitudes, including parallel formant amplitudes */
			...
			&& feacur != (FVOICD | FSONOR | FCONSON))
		{
		   ...
		   && pDphsettar->phcur != SPP_Z) {
			...
			if(((feacur & FPLOSV) IS_PLUS) || (pDphsettar->phcur == SPP_CH) || (pDphsettar->phcur == SPP_YH) || (pDphsettar->phcur == SPP_GH)) {
				if((feacur & FPLOSV) IS_PLUS) {
					if((feacur & FVOICD) IS_PLUS) {
		 ...
		 *      [s]->[z] / _ [+voice]
		 */
		if((pDphsettar->np == &PAV) && (pDphsettar->phcur == SPP_S || pDphsettar->phcur == SPP_TH) && ((feanex & (FCONSON | FVOICD)) == (FCONSON | FVOICD))) {
			pDphsettar->durtran = pDph_t->durfon >> 1;
		...
		if(((feacur & FNASAL) IS_PLUS) && ((feanex & FVOICD) IS_PLUS)) {
			pDphsettar->durtran = 0;
...
/* ***************** NOT USED **************************************/
		...
		if(((feanex & FPLOSV) IS_PLUS) || (pDphsettar->phonex == SPP_YH) || (pDphsettar->phonex == SPP_CH)) {
			pDphsettar->durtran = 0;
		...
		if(pDphsettar->np == &PTILT) {
			...
			if(((feacur & FVOICD) IS_PLUS) && ((feacur & FNASAL) IS_MINUS) && ((feanex & FVOICD) IS_MINUS)) {
				pDphsettar->durtran = NF50MS;
...
static void special_rules(LPTTS_HANDLE_T phTTS, short fealas, short feacur, short feanex,
	...
	if((feacur & FBURST) IS_PLUS) {
		if(pDphsettar->phcur == SPP_CH || pDphsettar->phcur == SPP_YH) {
			 ...
			 */
			if((fealas & FSONOR) IS_PLUS && pholas != GEN_SIL && bdur > 1) {
				bdur--;
			}
			/*
			 * Don't release plosive in plos-nasal or plos-plos sequence
			 */
			if(((feanex & (FNASAL | FPLOSV)) != 0) && sp_place[pDphsettar->phcur & PVALUE] >= sp_place[pDphsettar->phonex])
				bdur = 0;
		}
	...
	if((((fealas & FPLOSV) IS_PLUS) && ((fealas & FVOICD) IS_MINUS) && ((feacur & FSONOR) IS_PLUS))) {
		...
		switch(pDphsettar->phcur) { /* Vot of ptk before high vowel */
		case SPP_I:
		case SPP_U:
		case SPP_YX:
		case SPP_WX:
...
*/
		...
		if((feanex & (FNASAL | FPLOSV)) IS_PLUS) {
			...
			if((feacur & FPLOSV) IS_PLUS)
				bdur = 0;
			if((sp_place[pDphsettar->phcur & PVALUE] | MASKFRONT) == (sp_place[pDphsettar->phonex] | MASKFRONT)) { /* yes they're both done up front */
				bdur = 0;
			} else if((sp_place[pDphsettar->phcur & PVALUE]) == (sp_place[pDphsettar->phonex])) { /* yes they're both done up front */
				bdur = 0;
			}
		}
#endif
	...
	if(durin < quarterinhdr) {
		...
		if(shrink > FRAC_ONE) {
			...
			if(arg1 < 8193)
				arg1 = 8193;
			/* (3 * FRAC_ONE ) - (2 * shrink) */
			arg2 = ((((FRAC_ONE - arg1) + FRAC_ONE) - arg1) + FRAC_ONE);
		}
		if(arg2 <= 0)
			arg2 = 1;

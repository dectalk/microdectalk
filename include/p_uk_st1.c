...
short uk_gettar(LPTTS_HANDLE_T phTTS, int nphone_temp) {
	...
	if((pDphsettar->par_type = partyp[npar]) IS_FORM_FREQ_OR_BW) {
		...
		if(((phone_temp == UKP_N) || (phone_temp == UKP_EN)) && (npar == B2 - 1)) {
			if(begtyp(phnex_temp) != 1) {
				tartemp += 60;
			}
		}
		/* Special rule for B3 of \n/ adjacent to high-front vowels */
		if(((phone_temp == UKP_N) || (phone_temp == UKP_EN) || (phone_temp == UKP_NX)) && (npar == B3 - 1)) {
			if(((uk_place[phnex_temp & PVALUE] & F2BACKI) IS_PLUS) || ((uk_place[(phlas_temp & PVALUE)] & F2BACKF) IS_PLUS)) {
				tartemp = 1600;
			}
		}
	}
	...
	/* GETTAR:  AV, AP */
		...
		if(npar == AV - 1) {
			...
			if((phone_temp == UKP_HX) && ((phone_feature(pDph_t, phlas_temp) & FVOICD) IS_PLUS) && ((pDph_t->allofeats[nphone_temp] & FSTRESS_1) IS_MINUS)) {
				tartemp = 54;
			}
		}
	...
	/* GETTAR:  A2, A3, A4, A5, A6, AB */
		...
		if(pDphsettar->np == &PTILT) {
			...
			if(phone_temp == UKP_HX) {
				...
				tartemp = 7;
				if(((phone_feature(pDph_t, phone_temp) & FVOICD) IS_PLUS) && (((phone_feature(pDph_t, phone_temp) & FPLOSV) IS_PLUS) || (pDphsettar->phcur == UKP_JH))) {
...
static short UKP_special_coartic(PDPH_T pDph_t, short nfon, short diphpos) {
	...
	if(pDphsettar->np == &PF3) {
		if(((phone_feature(pDph_t, foncur) & FVOWEL) IS_PLUS) && (foncur != UKP_RR)) {
			if(((fonlas == UKP_W) || (fonlas == UKP_R)) || ((fonnex == UKP_W) || (fonnex == UKP_R))) {
				temp = -150;
			}
		}
...
static void uk_forw_smooth_rules(LPTTS_HANDLE_T phTTS, short shrif, short pholas, short fealas,
	...
	if(pDphsettar->par_type IS_FORM_FREQ) {
		...
		if(pDphsettar->phcur == GEN_SIL) {
			...
			if((feacur & FNASAL) IS_PLUS) {
				...
				else if(((pDphsettar->phcur == UKP_N) || (pDphsettar->phcur == UKP_EN)) && (endtyp(pholas) == 1)) {
					if(pDphsettar->np == &PF2) {
						pDphsettar->bouval -= 100;
				...
				else if((pDphsettar->np == &PF2) && (pDphsettar->phcur == UKP_M) && ((uk_place[pholas & PVALUE] & F2BACKF) IS_PLUS)) {
	...
	/* FORWARD SMOOTH:  B1, B2, B3 */
		...
		if((fealas & FNASAL) IS_PLUS) {
			...
			if((pDphsettar->np == &PB2) && ((pholas == UKP_N) || (pholas == UKP_EN)) && (begtyp(pDphsettar->phcur) != 1)) {
	...
	/* Default pDphsettar->bouval is average of tarcur & tarend, default tc is 30 ms */
		...
		if((pDphsettar->bouval < temp) || ((fealas & FPLOSV) IS_PLUS) || (pholas == UKP_JH)) {
		...
		if(pDphsettar->np == &PAP) {
			if((pDphsettar->phcur == GEN_SIL) || (pDphsettar->phcur == UKP_F) || (pDphsettar->phcur == UKP_TH) || (pDphsettar->phcur == UKP_S) || (pDphsettar->phcur == UKP_SH)) {
...
static void uk_back_smooth_rules(LPTTS_HANDLE_T phTTS, short shrib, short feacur,
	...
	if(pDphsettar->par_type IS_FORM_FREQ) {
		...
		if(pDphsettar->phonex == GEN_SIL) {
			...
			if((feacur & FNASAL) IS_PLUS) {
				...
				else if(((pDphsettar->phcur == UKP_N) || (pDphsettar->phcur == UKP_EN)) && (begtyp(pDphsettar->phonex) == 1)) {
					if(pDphsettar->np == &PF2) {
						pDphsettar->bouval -= 100;
				...
				else if((pDphsettar->np == &PF2) && (pDphsettar->phcur == UKP_M) && ((uk_place[pDphsettar->phonex & PVALUE] & F2BACKI) IS_PLUS)) {
					pDphsettar->bouval -= 150;
				}
#endif
			}
	...
	/* BACKWARD SMOOTH:  B1, B2, B3 */
		...
		if((feanex & FNASAL) IS_PLUS) {
			...
			if((pDphsettar->np == &PB2) && ((pDphsettar->phonex == UKP_N) || (pDphsettar->phonex == UKP_EN)) && (endtyp(pDphsettar->phcur) != 1)) {
				pDphsettar->bouval += 60;
				pDphsettar->durtran = NF60MS;
			}
			if(pDphsettar->np == &PB1) {
	...
	/* BACKWARD SMOOTH:  AV, AP, A2, A3, A4, A5, A6, AB */
		...
		if((pDphsettar->np == &PAV) && (pDphsettar->bouval < pDphsettar->np->tarnex) && (pDphsettar->phcur != UKP_V) && (pDphsettar->phcur != UKP_DH) && (pDphsettar->phcur != UKP_JH) && (pDphsettar->phcur != UKP_ZH) && (pDphsettar->phcur != UKP_Z)) {
			pDphsettar->durtran = 0;
		...
		if((pDphsettar->phonex >= UKP_P) && (((feacur & FNASAL) IS_MINUS) || (pDphsettar->np != &PAV))) {
			pDphsettar->durtran = 0;
		...
		if(pDphsettar->np == &PAP) {
			if(
			    ...
			    (pDphsettar->phcur == UKP_F) || (pDphsettar->phcur == UKP_TH) || (pDphsettar->phcur == UKP_S) || (pDphsettar->phcur == UKP_SH)) {
...
*/
		...
		if((feanex & (FNASAL | FPLOSV)) IS_PLUS) {
			...
			if((feacur & FPLOSV) IS_PLUS)
				bdur = 0;
			if((uk_place[pDphsettar->phcur & PVALUE] | MASKFRONT) == (uk_place[pDphsettar->phonex & PVALUE] | MASKFRONT)) { /* yes they're both done up front */
				bdur = 0;

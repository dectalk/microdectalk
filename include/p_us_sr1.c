	...
	for(m = msym + 1; m < pDph_t->nsymbtot; m++) {
		...
		if((sy == S1) || (sy == S2) || (sy == SEMPH)) {
			...
			if(mcl != 1) {
				...
				if((mcl == 3) && ((cl != CLUSTER_TRYS) || (pDph_t->symbols[m - 3] != USP_S)))
					return;
			}
			if(sy == S1) {
...
short us_phcluster(short f, short s) {
	...
	case USP_G:
		...
		break;
	case USP_S:
		if((s == USP_W) || (s == USP_LL) || (s == USP_P) || (s == USP_T) || (s == USP_K) || (s == USP_M) || (s == USP_N) || (s == USP_F))
			return (CLUSTER);
		break;
	case USP_SH:
		if((s == USP_W) || (s == USP_LL) || (s == USP_P) || (s == USP_T) || (s == USP_R) || (s == USP_M) || (s == USP_N))
			return (CLUSTER);
		break;
	}
	return (NOCLUSTER);

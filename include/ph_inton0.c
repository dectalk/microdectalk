...
void phinton(LPTTS_HANDLE_T phTTS) {
	...
	fbfound:
		...
		if((pDph_t->f0mode == PHONE_TARGETS_SPECIFIED) || (pDph_t->f0mode == SINGING)) {
...
// #if defined (ENGLISH) || defined (GERMAN)
...

					{
						...
						if((pDphsettar->hatsize >= 2000) || (pDphsettar->hatsize <= 0) || (inputscrewup == TRUE)) {
				...
				{	/* Primary or emph */
					...
					if(((struccur & FHAT_ENDS) IS_PLUS) || ((struccur & FPERNEXT) IS_PLUS)) {
					...
					if(pDph_t->f0mode == HAT_F0_SIZES_SPECIFIED) {
						...
						if((targf0 >= 2000) || (targf0 <= 0) || (inputscrewup == TRUE)) {
					...
					if(pDph_t->number_words > 2) {
						if(((struccur & FHAT_ENDS) IS_PLUS) || ((struccur & FPERNEXT) IS_PLUS)) /* last stressed syl of phrase*/
						{
	...
	duplication is so that the rules fire in the correct order also*/
				...
				if(pDph_t->had_hatend) {
					...
					bfound:
						...
						if(nextsylbou != nextphrbou) {
							...
							if((nextphrbou == FCBNEXT) || (nextphrbou == FQUENEXT)) {
						...
						/*			  i.e. readjust timing so get fall-rise, not r-f-r */
						else if((nextphrbou == FCBNEXT) || (nextphrbou == FQUENEXT)) {
							lowrisesw++;
							delayf0 = -NF20MS;
							f0fall	= 140;
						}
				...
				if(((struccur & FBOUNDARY) == FCBNEXT) || ((struccur & FBOUNDARY) == FQUENEXT)) {
		...
		if((feacur & FSYLL) IS_PLUS) {
			if(((stresscur & FSTRESS_1) IS_MINUS)	   /* 2-str or 0-str */
			   || ((struccur & FHAT_ENDS) IS_MINUS)) { /* or 1-str nofall */

#ifdef SPANISH
				if((struccur & FTYPESYL) >= FBISYL && ((struccur & FBOUNDARY) > FWBNEXT || (phone_feature(pDph_t, phonex) & FCONSON) IS_PLUS)) {
#endif

					/* Pitch falls (glottalize) at end of declar. sent. */
					if(((struccur & FBOUNDARY) == FPERNEXT) || ((struccur & FBOUNDARY) == FEXCLNEXT)) {
		...
		if(phocur == GEN_SIL) {
			...
			if((pDph_t->allofeats[nphon - 1] & FBOUNDARY) == FCBNEXT && pDph_t->nf0tot > 0 && (pDph_t->allophons[nphon - 1] != GEN_SIL)) {
	...
	skiprules: /* END OF F0 RULES */
		 ...
		 */
		if((phonex == GEN_SIL) && (phone_feature(pDph_t, phocur) & FPLOSV) && (phone_feature(pDph_t, phocur) & FBURST)) {
			/* p t k b d g */
			...
			if((begtyp(pholas) == 1) || (phone_feature(pDph_t, phocur) & FDENTAL)) {
...
// #if defined (ENGLISH_US) || defined (GERMAN)
				...
				if((stresscur & FSTRESS_1) IS_PLUS)
					...
					{ /* Primary or emph */
						/* Make stress impulse prop. to degree of stress */
						/* and stress position relative to clause onset */
						targf0 = us_f0_stress_level[stresscur] + us_f0_phrase_position[pDphsettar->nrises_sofar];
...
// #if defined ENGLISH_US || defined GERMAN
	...
	skiprules: /* END OF F0 RULES */
		 ...
		 * delaying it to here makes all dur and f0 rules much simpler
		 */
		if((phonex == GEN_SIL) && (((phocur >= USP_P) && (phocur <= USP_G)) /* p t k b d g */
					   /* || ((feacur & FNASAL) IS_PLUS) */)
		   /* m n nx en */
		 ...
		 *   delaying it to here makes all dur and f0 rules much simpler */
		if((phonex == GEN_SIL) && ((feacur & FPLOSV) IS_PLUS) && (pDph_t->nallotot < NPHON_MAX)) {
			for(n = pDph_t->nallotot; n > nphon; n--) {
				pDph_t->allophons[n] = pDph_t->allophons[n - 1];
				pDph_t->allofeats[n] = pDph_t->allofeats[n - 1];
				pDph_t->allodurs[n]  = pDph_t->allodurs[n - 1];

...
void us_phtiming(LPTTS_HANDLE_T phTTS) {
	...
	for(nphon = 0; nphon < pDph_t->nallotot; nphon++) {
		...
		if(strucboucur >= FCBNEXT) {
			...
			if(((phocur == USP_RX) || (phocur == USP_LX)) && ((pDphsettar->feanex & FOBST) IS_PLUS) && ((pDphsettar->feanex & FVOICD) IS_MINUS)) {
				deldur = NF15MS;
			}
		...
		if(feasyllabiccur IS_PLUS) {
			if(((strucstresscur & FSTRESS_1) IS_MINUS) && ((struccur & FTYPESYL) == FMONOSYL)) {
		...
		if((feasyllabiccur IS_PLUS) || ((phocur >= USP_RX) && (phocur <= USP_NX) && ((struccur & (FSTRESS | FWINITC)) IS_MINUS) && ((pDphsettar->feanex & FOBST) IS_PLUS))) {
			...
			if(((pDphsettar->feanex & FSYLL) IS_MINUS) && ((pDphsettar->strucnex & (FSTRESS | FWINITC)) IS_MINUS)) {
				...
				if(((posvoc >= USP_RX) && (posvoc <= USP_NX)) && ((phone_feature(pDph_t, pDph_t->allophons[nphon + 2]) & FOBST) IS_PLUS) && ((pDph_t->allofeats[nphon + 2] & (FSTRESS | FWINITC)) IS_MINUS)) {
					psonsw = 1;
				...
				if(posvoc != GEN_SIL) {
					if((phone_feature(pDph_t, posvoc) & FVOICD) IS_MINUS) {
						...
						arg1 = N80PRCNT;
						if(((phone_feature(pDph_t, posvoc) & FPLOSV) IS_PLUS) || (posvoc == USP_CH)) {
					...
					/* Postvocalic segment is voiced */
						...
						if((phone_feature(pDph_t, posvoc) & FOBST) IS_PLUS) {
							...
							if(((phone_feature(pDph_t, posvoc) & FPLOSV) IS_MINUS)
							   ...
							   && (posvoc != USP_DX) && ((feacur & FSYLL) IS_PLUS)) {
								deldur = deldur + NF25MS;
							}
			...
			if(((phocur == USP_N) && (pDphsettar->phonex_timing == USP_T)) && ((pDphsettar->strucnex & (FWINITC | FSTRESS)) IS_MINUS)) {

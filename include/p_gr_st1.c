...
short gr_gettar(LPTTS_HANDLE_T phTTS, int nphone_temp) {
	...
	if((pDphsettar->par_type = partyp[npar]) IS_FORM_FREQ_OR_BW) {
		...
		if((npar == F1 - 1) && phone_temp == GRP_KH)
			//			  tartemp -= 400;
			if((npar == F2 - 1) && phone_temp == GRP_KH)
				tartemp -= 600;
		...
		if(((phone_temp == GRP_N) || (phone_temp == GRP_EN)) && (npar == B2 - 1)) {
	...
	/* GETTAR:  A2, A3, A4, A5, A6, AB */
		...
		if(pDphsettar->np == &PTILT) {
			...
			if(phone_temp == GEN_SIL) {
				...
				tartemp = 7;
				if(((phone_feature(pDph_t, phone_temp) & FVOICD) IS_PLUS) && (((phone_feature(pDph_t, phone_temp) & FPLOSV) IS_PLUS) || (pDphsettar->phcur == GRP_DJ))) {
...
static short gr_special_coartic(PDPH_T pDph_t, short nfon, short diphpos) {
	...
	if(pDphsettar->np == &PF2) {
		...
		if(fonnex == GRP_L) {
			if((foncur == GRP_E) && (foncur == GRP_AE) || (foncur == GRP_I) || (foncur == GRP_EH) || (foncur == GRP_AEH) || (foncur == GRP_IH)) {
				temp = -150;
			}
			if(((foncur == GRP_AU) || (foncur == GRP_EU)) && (diphpos == 1)) {
				temp = -250;
			}
			if(((foncur == GRP_AU) || (foncur == GRP_EU)) && (diphpos > 1)) {
				temp = -350;
		...
		if((foncur == GRP_UE) || ((foncur == GRP_U) && (diphpos > 0))) {
...
static void gr_forw_smooth_rules(LPTTS_HANDLE_T phTTS, short shrif, short pholas,
	...
	if(pDphsettar->par_type IS_FORM_FREQ) {
		...
		if(pDphsettar->phcur == GEN_SIL) {
			...
			if((feacur & FOBST) IS_PLUS && pDphsettar->phcur != GRP_L) {
				pDphsettar->durtran = NF30MS;
				if(pDphsettar->np == &PF1)
	...
	/* Default pDphsettar->bouval is average of tarcur & tarend, default tc is 30 ms */
		...
		if((pDphsettar->bouval < temp) || ((fealas & FPLOSV) IS_PLUS) || (pholas == GRP_DJ)) {
...
static void gr_back_smooth_rules(LPTTS_HANDLE_T phTTS, short shrib, short feacur,
	...
	if(pDphsettar->par_type IS_FORM_FREQ) {
		...
		if(pDphsettar->phonex == GEN_SIL) {
			...
			/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
			if((feacur & FPLOSV) IS_PLUS && pDphsettar->phcur != GRP_L) {
				pDphsettar->durtran = NF25MS;
			}
#ifdef NOTABELIEVER
			/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
			...
			if((feacur & FNASAL) IS_PLUS) {
				...
				if((pDphsettar->np == &PF2) && ((gr_place[pDphsettar->phonex & PVALUE] & F2BACKI) IS_PLUS)) {
		...
		if(((feacur & FOBST) IS_MINUS) && (gr_begtyp[pDphsettar->phonex] != 4) /* Unless next seg is obst */
		   && (pDphsettar->durtran > 0)) {
			// pDph_t->arg1 = pDphsettar->durtran;
			// pDph_t->arg2 = shrib;
	...
	/* BACKWARD SMOOTH:  AV, AP, A2, A3, A4, A5, A6, AB */
		...
		/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
		if((pDphsettar->phonex >= GRP_P) && (((feacur & FNASAL) IS_MINUS) || (pDphsettar->np != &PAV))) {
			pDphsettar->durtran = 0;
...
groupings" */
		...
		if((feanex & (FNASAL | FPLOSV)) IS_PLUS) {
			...
			if((gr_place[pDphsettar->phcur & PVALUE] | MASKFRONT) == (gr_place[pDphsettar->phonex & PVALUE] | MASKFRONT)) { /* yes they're both done up front */
				bdur = 0;
			} else if(gr_place[pDphsettar->phcur & PVALUE] == gr_place[pDphsettar->phonex & PVALUE]) { /* yes they're both done up front */
				bdur = 0;
			}
		}
#endif

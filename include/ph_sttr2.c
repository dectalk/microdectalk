...

{
	...
	} else { /* Or at the end of the sonorant */
		typob = begtyp(fonobst);
		typso = endtyp(fonsonor);
	}
#ifdef needs_completion // HELP
	if(fonobst == GRP_L || fonobst == USP_LL || fonobst == USP_LX) {
		typob = OBSTRUENT;
		if(typso == OBSTRUENT)
	...
	if((typso == ROUNDED_SONOR_CONS) && (pDphsettar->np > &PF1) && ((place(fonobst) & (FPALATL | FDENTAL)) IS_MINUS)) {
		prcnt = (prcnt >> 1) + 50;
	}
	...
	if(((phone_feature(pDph_t, fonsonor) & FVOWEL) IS_PLUS) && ((phone_feature(pDph_t, fonvowel) & FVOWEL) IS_PLUS) && (pDphsettar->np == &PF2)) {

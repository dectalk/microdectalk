...
short la_gettar(LPTTS_HANDLE_T phTTS, int nphone_temp) {
	...
	if ((pDphsettar->par_type = partyp[npar]) IS_FORM_FREQ_OR_BW)
	{
		...
		if((npar == (B3 - F1)) && (phone_temp == LAP_N || phone_temp == LAP_NH || phone_temp == LAP_NX) && (((la_place[phnex_temp & PVALUE] & F2BACKI) IS_PLUS) || ((la_place[phlas_temp & PVALUE] & F2BACKI) IS_PLUS))) {
		 ...
		 */
		if((npar == (B3 - F1)) && ((la_place[phone_temp & PVALUE] & F2BACKI) IS_PLUS) && (phlas_temp == LAP_F)) {
			tartemp = 90;
		}
		/*
		 * Special rule for /r/ and /rr/ after "back" vowel.        8-Jul-86
		 */
		if((phone_temp == LAP_R || phone_temp == LAP_RR) && (phlas_temp == LAP_O || phlas_temp == LAP_U)) {
			switch(npar) {
			case(F1 - F1):
				tartemp -= 100;
				break;
	...
	else if (pDphsettar->par_type IS_PARALLEL_FORM_AMP)
	{
		...
		if(pDphsettar->np == &PTILT) {
			...
			else if((phone_feat & (FVOICD | FPLOSV)) == (FVOICD | FPLOSV) && ((phone_temp != LAP_DH) || (phone_temp == LAP_YH))) {
				tartemp = 12;
			...
			else if((phone_temp >= LAP_DH) && (phone_temp <= LAP_GH)) {
				tartemp = 24;
			}
...

{
	...
	if(pDphsettar->np == &PF1) {
		...
		case LAP_O:
			...
			if(loc == FLABIAL && other != LAP_F)
				return (-50);
#endif
			if(other == LAP_Y || other == LAP_YH)
				return (100);
			break;
		...
		case LAP_U:
			...
			/* OUT to prevent overload                              30-Apr-86       */
			/* Back in again                                        30-Jul-86       */
			if(other == LAP_M || other == LAP_F)
				return (-50);
#endif
			if(loc == FDENTAL || other == LAP_S || loc == FPALATL)
				return (75);
			break;
		}
		...
		if((feacur & FSONOR) IS_PLUS) {
			if((feacur & FSONCON) IS_MINUS) {
				...
				if((fealas & FSONCON) IS_PLUS) {
					...
					if(pDphsettar->phcur != GEN_SIL && (pholas == LAP_L) && (pDphsettar->np == &PF1)) {
...
/* OUT                  xx += ((bouval - np->tarend) >> 1); np->tarend += xx;   (DOESN'T WORK) END OUT */
			...
			if((feacur & FNASAL) IS_PLUS) {
				...
				else if(((pDphsettar->phcur == LAP_N)) && (begtyp(pDphsettar->phonex) == 1)) {
				...
				else if((pDphsettar->np == &PF2) && (pDphsettar->phcur == LAP_M || pDphsettar->phcur == LAP_MX) && ((la_place[pholas & PVALUE] & F2BACKF) IS_PLUS)) {
	...
	/* Formant bandwidths */
		...
		/* Special rule for [fi] */
		if((pDphsettar->np == &PB3) && ((la_place[pDphsettar->phcur & PVALUE] & F2BACKI) IS_PLUS) && (pholas == LAP_F)) {
			pDphsettar->durtran = 0;
	...
	/* Default is max of [average,tarcur-12,tarlas-12] */
		...
		if((pDphsettar->bouval < temp) || ((fealas & FPLOSV) IS_PLUS) || (pholas == LAP_YH)) { /* 7-May-86    */
...
/* ****************** NOT USED *************************************/
		  ...
		  NOTE:[y] and[yh] are now     obstruents
			...
			if((feacur & FSONCON) IS_MINUS) {
				if((feanex & FSONCON) IS_PLUS) {
					...
					if(pDphsettar->phcur != GEN_SIL && (pDphsettar->phonex == LAP_L) && (pDphsettar->np == &PF1)) {
						pDphsettar->bouval += 80;
					}
				} else {
				}
		...
		if(pDphsettar->phonex == GEN_SIL) {
			...
			if((feacur & FNASAL) IS_PLUS) {
				...
				else if((pDphsettar->np == &PF2) && (pDphsettar->phcur == LAP_M || pDphsettar->phcur == LAP_MX) && ((la_place[pDphsettar->phonex & PVALUE] & F2BACKI) IS_PLUS)) {
					pDphsettar->bouval -= 150;
				}
#endif
			}
	...
	/* Source amplitudes, including parallel formant amplitudes */
			...
			&& feacur != (FVOICD | FSONOR | FCONSON))
		{
		   ...
		   && pDphsettar->phcur != LAP_Z) {
			...
			if(((feacur & FPLOSV) IS_PLUS) || (pDphsettar->phcur == LAP_CH) || (pDphsettar->phcur == LAP_YH) || (pDphsettar->phcur == LAP_GH)) {
				if((feacur & FPLOSV) IS_PLUS) {
					if((feacur & FVOICD) IS_PLUS) {
		 ...
		 *      [s]->[z] / _ [+voice]
		 */
		if((pDphsettar->np == &PAV) && (pDphsettar->phcur == LAP_S || pDphsettar->phcur == LAP_TH) && ((feanex & (FCONSON | FVOICD)) == (FCONSON | FVOICD))) {
			pDphsettar->durtran = pDph_t->durfon >> 1;
...
/* ***************** NOT USED **************************************/
		...
		if(((feanex & FPLOSV) IS_PLUS) || (pDphsettar->phonex == LAP_YH) || (pDphsettar->phonex == LAP_CH)) {
			pDphsettar->durtran = 0;
...
static void special_rules(LPTTS_HANDLE_T phTTS, short fealas, short feacur, short feanex,
	...
	if((feacur & FBURST) IS_PLUS) {
		if(pDphsettar->phcur == LAP_CH || pDphsettar->phcur == LAP_YH) {
			 ...
			 */
			if(((feanex & (FNASAL | FPLOSV)) != 0) && la_place[pDphsettar->phcur & PVALUE] >= la_place[pDphsettar->phonex])
				bdur = 0;
		}
...
*/
		...
		if((feanex & (FNASAL | FPLOSV)) IS_PLUS) {
			...
			if((feacur & FPLOSV) IS_PLUS)
				bdur = 0;
			if((la_place[pDphsettar->phcur & PVALUE] | MASKFRONT) == (la_place[pDphsettar->phonex] | MASKFRONT)) { /* yes they're both done up front */
				bdur = 0;
			} else if((la_place[pDphsettar->phcur & PVALUE]) == (la_place[pDphsettar->phonex])) { /* yes they're both done up front */
				bdur = 0;
			}
		}
#endif

...
short us_gettar(LPTTS_HANDLE_T phTTS, int nphone_temp) {
	...
	if((pDphsettar->par_type = partyp[npar]) IS_FORM_FREQ_OR_BW) {
		...
		if((npar == F1 - 1) && ((phone_feature(pDph_t, phone_temp) & FOBST) IS_PLUS) && ((phone_feature(pDph_t, phone_temp) & FSTOP) IS_MINUS) && ((phone_feature(pDph_t, phlas_temp) & FSYLL) IS_PLUS)) {
			tartemp += 40;
		}
		/* Special rule for B2 of \n/ before non-front vowels */
		if(((phone_temp == USP_N) || (phone_temp == USP_EN)) && (npar == B2 - 1)) {
			if(begtyp(phnex_temp) != 1) {
				tartemp += 60;
			}
		}
		/* Special rule for B3 of \n/ adjacent to high-front vowels */
		if(((phone_temp == USP_N) || (phone_temp == USP_EN) || (phone_temp == USP_NX)) && (npar == B3 - 1)) {
			if(((us_place[phnex_temp & PVALUE] & F2BACKI) IS_PLUS) || ((us_place[phlas_temp & PVALUE] & F2BACKF) IS_PLUS)) {
				tartemp = 1600;
			}
		}
	}
	...
	/* GETTAR:  AV, AP */
		...
		if(npar == AV - 1) {
			...
			if(((phone_feature(pDph_t, phone_temp) & FPLOSV) IS_PLUS) && ((phone_feature(pDph_t, phlas_temp) & FVOICD) IS_MINUS)) {
				tartemp = 0;
			}
			/* Voice [h] if unstressed and preceeded by voiced seg */
			if((phone_temp == USP_HX) && ((phone_feature(pDph_t, phlas_temp) & FVOICD) IS_PLUS) && ((pDph_t->allofeats[nphone_temp] & FSTRESS_1) IS_MINUS)) {
				tartemp = 54;
			}
	...
	/* GETTAR:  A2, A3, A4, A5, A6, AB */
		...
		if(pDphsettar->np == &PTILT) {
			...
			if(phone_temp == USP_HX) {
				...
				tartemp = 7;
				if(((phone_feature(pDph_t, phone_temp) & FVOICD) IS_PLUS) && (((phone_feature(pDph_t, phone_temp) & FPLOSV) IS_PLUS) || (pDphsettar->phcur == USP_JH))) {
...
static short us_special_coartic(PDPH_T pDph_t, short nfon, short diphpos) {
	...
	if(pDphsettar->np == &PF3) {
		if(((phone_feature(pDph_t, foncur) & FVOWEL) IS_PLUS) && (foncur != USP_RR)) {
			if(((fonlas == USP_W) || (fonlas == USP_R) || (fonlas == USP_RX)) || ((fonnex == USP_W) || (fonnex == USP_R) || (fonnex == USP_RX))) {
				temp = -150;
			}
		}
	}
	...
	if(pDphsettar->np == &PF2) {
		...
		if(fonnex == USP_LX) {
			...
			if(((foncur == USP_AY) || (foncur == USP_OY)) && (diphpos == 1)) {
				temp = -250;
			}
			if(((foncur == USP_AY) || (foncur == USP_OY)) && (diphpos > 1)) {
				temp = -350;
		...
		if((foncur == USP_UW) || ((foncur == USP_YU) && (diphpos > 0))) {
...
static void us_forw_smooth_rules(LPTTS_HANDLE_T phTTS, short shrif, short pholas,
	...
	if(pDphsettar->par_type IS_FORM_FREQ) {
		...
		if(pDphsettar->phcur == GEN_SIL) {
			...
			if(((fealas & FPLOSV) IS_PLUS) && ((fealas & FVOICD) IS_MINUS)) {
			...
			if((feacur & FNASAL) IS_PLUS) {
				...
				else if(((pDphsettar->phcur == USP_N) || (pDphsettar->phcur == USP_EN)) && (endtyp(pholas) == 1)) {
					if(pDphsettar->np == &PF2) {
						pDphsettar->bouval -= 100;
				...
				else if((pDphsettar->np == &PF2) && (pDphsettar->phcur == USP_M) && ((us_place[pholas & PVALUE] & F2BACKF) IS_PLUS)) {
					pDphsettar->bouval -= 150;
				}
			}
		}
		/* Shrink transition dur inside sonor if sonor short */
		if(((feacur & FOBST) IS_MINUS) && (endtyp(pholas) != OBSTRUENT) /* Unless prev seg is obst */
		   && (pDphsettar->durtran > 0)) {
	...
	/* FORWARD SMOOTH:  FN */
		...
		pDphsettar->durtran = 0;
		if(((fealas & FNASAL) IS_PLUS) && ((feacur & FNASAL) IS_MINUS)) {
	...
	/* FORWARD SMOOTH:  B1, B2, B3 */
		...
		if((feacur & FVOICD) IS_PLUS) {
			if((pDphsettar->np == &PB1) && ((fealas & FVOICD) IS_MINUS)) {
		...
		if(pholas == GEN_SIL) {
			...
			if(((phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone - 2]) & FVOICD) IS_MINUS) && ((struclas & FDUMMY_VOWEL) IS_PLUS) && (pDphsettar->np == &PB1)) {
		...
		if((fealas & FNASAL) IS_PLUS) {
			...
			if((pDphsettar->np == &PB2) && ((pholas == USP_N) || (pholas == USP_EN)) && (begtyp(pDphsettar->phcur) != 1)) {
	...
	/* Default pDphsettar->bouval is average of tarcur & tarend, default tc is 30 ms */
		...
		if((pDphsettar->bouval < temp) || ((fealas & FPLOSV) IS_PLUS) || (pholas == USP_JH)) {
		...
		if(((fealas & FNASAL) IS_PLUS) && ((feacur & FVOICD) IS_PLUS)) {
			pDphsettar->durtran = 0;
		...
		if(pDphsettar->np == &PAP) {
			if((pDphsettar->phcur == GEN_SIL) || (pDphsettar->phcur == USP_F) || (pDphsettar->phcur == USP_TH) || (pDphsettar->phcur == USP_S) || (pDphsettar->phcur == USP_SH)) {
				if(((fealas & FVOICD) IS_PLUS) && ((fealas & FOBST) IS_MINUS)) {
					if(pDphsettar->phcur == GEN_SIL) {
		...
		if(pDphsettar->np == &PTILT) {
			...
			if(((fealas & FSTOP) IS_PLUS) || ((feacur & FSTOP) IS_PLUS)) {
				pDphsettar->durtran = 0;
			}
		}
	}
...
static void us_back_smooth_rules(LPTTS_HANDLE_T phTTS, short shrib, short feacur,
	...
	if(pDphsettar->par_type IS_FORM_FREQ) {
		...
		if(pDphsettar->phonex == GEN_SIL) {
			...
			if((feacur & FNASAL) IS_PLUS) {
				...
				else if(((pDphsettar->phcur == USP_N) || (pDphsettar->phcur == USP_EN)) && (begtyp(pDphsettar->phonex) == 1)) {
					if(pDphsettar->np == &PF2) {
						pDphsettar->bouval -= 100;
				...
				else if((pDphsettar->np == &PF2) && (pDphsettar->phcur == USP_M) && ((us_place[pDphsettar->phonex & PVALUE] & F2BACKI) IS_PLUS)) {
					pDphsettar->bouval -= 150;
				}
			}
		}
		/* Shrink tran dur inside sonorant if sonorant short */
		if(((feacur & FOBST) IS_MINUS) && (begtyp(pDphsettar->phonex) != 4) /* Unless next seg is obst */
		   && (pDphsettar->durtran > 0)) {
			pDph_t->arg1	    = pDphsettar->durtran;
			pDph_t->arg2	    = shrib;
			pDphsettar->durtran = mlsh1(pDph_t->arg1, pDph_t->arg2) + 1;
	...
	/* BACKWARD SMOOTH:  FN */
		...
		if(((feanex & FNASAL) IS_PLUS) && ((feacur & FNASAL) IS_MINUS)) {
			pDphsettar->bouval  = 400;
			pDphsettar->durtran = NF80MS;
	...
	/* BACKWARD SMOOTH:  B1, B2, B3 */
		...
		if((feanex & FNASAL) IS_PLUS) {
			...
			if((pDphsettar->np == &PB2) && ((pDphsettar->phonex == USP_N) || (pDphsettar->phonex == USP_EN)) && (endtyp(pDphsettar->phcur) != 1)) {
				pDphsettar->bouval += 60;
				pDphsettar->durtran = NF60MS;
			}
			if(pDphsettar->np == &PB1) {
	...
	/* BACKWARD SMOOTH:  AV, AP, A2, A3, A4, A5, A6, AB */
		...
		if((pDphsettar->np == &PAV) && (pDphsettar->bouval < pDphsettar->np->tarnex) && (pDphsettar->phcur != USP_V) && (pDphsettar->phcur != USP_DH) && (pDphsettar->phcur != USP_JH) && (pDphsettar->phcur != USP_ZH) && (pDphsettar->phcur != USP_Z)) {
			pDphsettar->durtran = 0;
		...
		if(((feanex & FNASAL) IS_PLUS) && ((feacur & FVOICD) IS_PLUS)) {
			pDphsettar->durtran = 0;
			// debugforward (" AV const in voiced to  nasal", pDphsettar->np);
		}
		/* If curr phone nasal, and next is voiced non-obst, AV const */
		if((feacur & FNASAL) IS_PLUS) {
			if(((feanex & FVOICD) IS_PLUS) && ((feanex & FOBST) IS_MINUS) && ((strucnex & FDUMMY_VOWEL) IS_MINUS)) {
				pDphsettar->durtran = 0;
		...
		if(pDphsettar->bouval < temp) { /* Or does it ??? */
			pDphsettar->bouval  = temp - 3;
			pDphsettar->durtran = NF20MS;
		}
		...
		if(pDphsettar->np == &PAV) {
			if((pDphsettar->bouval < temp) || ((temp > 0) && (pDphsettar->np == &PAV) && ((strucnex & FDUMMY_VOWEL) IS_PLUS))) {
				pDphsettar->bouval = temp + 3;
				if((pDphsettar->phonex == GEN_SIL) || ((strucnex & FDUMMY_VOWEL) IS_PLUS)) {
		...
		if((pDphsettar->phonex >= USP_P) && (((feacur & FNASAL) IS_MINUS) || (pDphsettar->np != &PAV))) {
			pDphsettar->durtran = 0;
		...
		if(pDphsettar->np == &PAP) {
			if(
			    ...
			    (pDphsettar->phcur == USP_F) || (pDphsettar->phcur == USP_TH) || (pDphsettar->phcur == USP_S) || (pDphsettar->phcur == USP_SH)) {
				if(((feanex & FVOICD) IS_PLUS) && ((feanex & FOBST) IS_MINUS)) {
					pDphsettar->bouval  = 52;
					pDphsettar->durtran = NF40MS;
				}
			}
			/* Offset of a vowel into silence is breathy */
			if(((feacur & FSYLL) IS_PLUS) && (pDphsettar->phonex == GEN_SIL)) {
				pDphsettar->bouval  = 52;
				pDphsettar->durtran = NF130MS;
			}
		}
		...
		if(pDphsettar->np == &PTILT) {
			...
			if(((feanex & FSTOP) IS_PLUS) || ((feacur & FSTOP) IS_PLUS)) {
				pDphsettar->durtran = 0;
			}
			/* Long breathy offset into silence */
			if(((feacur & FVOICD) IS_PLUS) && ((feacur & FNASAL) IS_MINUS)) {
				if(pDphsettar->phonex == GEN_SIL) {
...
static void us_special_rules(LPTTS_HANDLE_T phTTS, short fealas,
	...
	if((feacur & FBURST) IS_PLUS) {
		...
		if((feanex & (FNASAL | FPLOSV)) IS_PLUS) {
			if(us_place[pDphsettar->phcur & PVALUE] == us_place[pDphsettar->phonex & PVALUE]) { /* Was >= */
				bdur = 0;
			}
		}

		if(bdur > 1) {
			if(((feacur & FPLOSV) IS_PLUS) && ((feanex & FOBST) IS_PLUS)) {
	...
	vot = 0;
	if((((fealas & FPLOSV) IS_PLUS) && ((fealas & FVOICD) IS_MINUS) && ((feacur & FSONOR) IS_PLUS))) {
		...
		if((vot > (pDph_t->durfon >> 1)) && ((feacur & FSYLL) IS_PLUS) && ((struccur & FSTRESS_1) IS_PLUS)) {
			vot = pDph_t->durfon >> 1;
		}
	...
	if(((feacur & FBURST) IS_PLUS) && ((feacur & FVOICD) IS_PLUS) && ((fealas & FVOICD) IS_PLUS) && ((feanex & FVOICD) IS_MINUS) /* Voicebar only if next -V */

...
void sp_phtiming(LPTTS_HANDLE_T phTTS) {
  ...
  /* MAIN LOOP, for each output phoneme */
	...
	for(nphon = 0; nphon < pDph_t->nallotot; nphon++) {
		...
		if((fealas & FCONSON) IS_PLUS && strboulas < FVPNEXT) {
			durmin -= (durmin >> 2);
			// prdurs (pDph_t, phocur, durinh, durmin, deldur, prcnt, 102);
		} else if(((feacur & FVDSTOP) == FVDSTOP) && (pholas == GEN_SIL || strboulas >= FVPNEXT) && (pDphsettar->phonex_timing == SPP_O || pDphsettar->phonex_timing == SPP_U)) {
			durmin >>= 1;
			durinh -= (durinh >> 2);
			// prdurs (pDph_t, phocur, durinh, durmin, deldur, prcnt, 103);
		}

		if((feacur & FNASAL) IS_PLUS && ((fealas & FNASAL) IS_PLUS || (pDphsettar->feanex & FNASAL) IS_PLUS)) {
			durmin -= (durmin >> 2);
		...
		if((feacur & FSYLL) IS_PLUS) {
			...
			} else if(strucboucur == FWBNEXT && strucstresscur == FSTRESS_1) {
				/*
			 ...
			 */
			if((phocur == pDphsettar->phonex_timing && strucboucur < FPPNEXT && (struccur & FSBOUND) IS_MINUS) || (phocur == pholas && strboulas < FPPNEXT && (struclas & FSBOUND) IS_MINUS)) {
			 ...
			 * is yx or wx.)
			 */
			else if(((feacur & FVOWEL) IS_PLUS) && ((struccur & FSBOUND) IS_MINUS) && ((pDphsettar->feanex & FSEMIV) IS_PLUS)) {
				...
				if((struccur & FTYPESYL) == FMONOSYL) {
					...
					if((strucstresscur & FSTRESS_1) IS_MINUS) {
						...
						if(strucstresscur IS_MINUS) {
							...
							//  of anything by las......
							if(pDph_t->allophons[nphon - 1] == SPP_L && pDph_t->allophons[nphon] == SPP_A && pDph_t->allophons[nphon + 1] != SPP_S) {
			...
			if(strucstresscur == FNOSTRESS) {
				if((((pDphsettar->feanex & FSYLL) IS_PLUS) && ((pDphsettar->strucnex & FSBOUND) IS_MINUS) && (strucboucur == FNO_BOUNDARY)) || (((fealas & FSYLL) IS_PLUS) && ((struccur & FSBOUND) IS_MINUS) && (strboulas == FNO_BOUNDARY))) {
		...
		if((feacur & FCONSON) IS_PLUS) {
			...
			if(phocur == SPP_S) {
				...
				if((strucopensyl IS_PLUS)	   /* Open syll.      */
				   || (strucsylcur != FMONOSYL	   /* or end of       */
					...
					temp = N92PRCNT;
				else if((strucopensyl IS_MINUS) && strucboucur < FWBNEXT)
					temp = N70PRCNT;
				...
				temp = N100PRCNT;
				if(phocur == SPP_R) {
					if((strucboucur >= FMBNEXT) || ((pDphsettar->feanex & FCONSON) IS_PLUS))
						temp = N150PRCNT;
					else if(pholas == SPP_S || pholas == SPP_J || pholas == SPP_TH)
						temp = N60PRCNT;
				} else
				/* if (phocur == SPP_RR) */
				{
			...
			} else if((phocur == SPP_L || phocur == SPP_N) && (strucopensyl IS_PLUS))

			{
				 ...
				 */
				temp = N100PRCNT;
				if((pholas == SPP_N) && (feacur & FBURST) IS_PLUS)
					temp = N70PRCNT;
				else if(((feacur & FBURST) IS_PLUS) && (((fealas & FSONOR) IS_PLUS) && pholas != GEN_SIL) && (strucstresscur == FNOSTRESS))
					temp = N70PRCNT;
				else if((pDphsettar->phonex_timing == SPP_R || pDphsettar->phonex_timing == SPP_L) && (strucopensyl IS_PLUS))
					temp = N90PRCNT;
			...
			if((feacur & FNASAL) IS_PLUS) {
				if((phocur != SPP_NH) && ((struccur & FBOUNDARY) >= FVPNEXT)) {
					prcnt = mlsh1(N125PRCNT, prcnt);
...
#ifdef NOTYET
			if((phone_feature(pDph_t, pDph_t->allophons[nphon + 1]) & WBOUND) IS_PLUS && pDph_t->allophons[nphon] == SPP_S) {

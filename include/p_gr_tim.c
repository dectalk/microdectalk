...
void gr_phtiming(LPTTS_HANDLE_T phTTS) {
	...
	for(nphon = 0; nphon < pDph_t->nallotot; nphon++) {
		...
		if(phocur == GRP_R && (struccur & FBOUNDARY) >= FWBNEXT) {
			prcnt -= 50;
		}
		...
		if((feasyllabiccur IS_PLUS) && ((struccur & (FSTRESS | FWINITC)) IS_MINUS) && ((pDphsettar->feanex & FOBST) IS_PLUS)) {
			...
			if(((pDphsettar->feanex & FSYLL) IS_MINUS) && ((pDphsettar->strucnex & (FSTRESS | FWINITC)) IS_MINUS)) {
				...
				if((((phone_feature(pDph_t, pDph_t->allophons[nphon + 1]) & FCONSON) IS_PLUS) && ((phone_feature(pDph_t, pDph_t->allophons[nphon + 1]) & FSONOR) IS_PLUS)) && ((phone_feature(pDph_t, pDph_t->allophons[nphon + 2]) & FOBST) IS_PLUS) && ((pDph_t->allofeats[nphon + 2] & (FSTRESS | FWINITC)) IS_MINUS)) {
					psonsw = 1;
				...
				if(posvoc != GEN_SIL) {
					if((phone_feature(pDph_t, posvoc) & FVOICD) IS_MINUS) {
						...
						arg1 = N80PRCNT;
						if(((phone_feature(pDph_t, posvoc) & FPLOSV) IS_PLUS) || (posvoc == GRP_CH)) {
			...
			if(((phocur == GRP_N) && (pDphsettar->phonex_timing == GRP_T)) && ((pDphsettar->strucnex & (FWINITC | FSTRESS)) IS_MINUS)) {
		...
		/* Rule 13: Shorten consonant clusters */
			...
			if((feacur & FCONSON) IS_PLUS && (pholas != GRP_S)) {
		...
		if((feacur & FVOWEL) IS_PLUS) {
			if(((fealas & FSON2) IS_PLUS) && ((fealas & FNASAL) IS_MINUS) && pholas != GRP_RR) {
				if(deldur == 0)
					deldur = NF20MS;
			}
		}
		...
		if(pDph_t->allophons[nphon - 1] == GRP_TS && (feacur & FVOWEL) IS_PLUS) {
		...
		*/
		...
		if((pDph_t->allofeats[nphon + 1] & FDOUBLECONS) && ((struccur & FTYPESYL) != FMONOSYL)) {
			prcnt -= 40;
			if(prcnt <= 0)
				prcnt = 1;
		}

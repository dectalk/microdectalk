...
void uk_phtiming(LPTTS_HANDLE_T phTTS) {
	...
	for(nphon = 0; nphon < pDph_t->nallotot; nphon++) {
		...
		if(strucboucur >= FCBNEXT && pDph_t->number_words >= 4) {
			deldur = NF40MS;
			/* Except for plosives and voiced fricatives */
			if(((feacur & FVOICD) IS_PLUS) && ((feacur & FOBST) IS_PLUS)) {
				deldur = NF20MS;
			}
			...
			if(phocur == UKP_LX && ((pDphsettar->feanex & FOBST) IS_PLUS) && ((pDphsettar->feanex & FVOICD) IS_MINUS)) {
				deldur = NF15MS;
			}
			/* More lengthening of a vowel if in a short phrase */
			if((pDph_t->nallotot < 10) && (feasyllabiccur IS_PLUS) && (strucstresscur IS_PLUS)) {
		...
		if(feasyllabiccur IS_PLUS) {
			if(((strucboucur < FVPNEXT) && (pKsd_t->sprate > 160)) || (strucboucur < FPPNEXT)) {
		...
		if(((feacur & FNASAL) IS_PLUS && fealas & FNASAL) IS_PLUS)
			if(phocur != pholas)
		...
		if(feasyllabiccur IS_PLUS) {
			...
			} else if(((struccur & FTYPESYL) != FMONOSYL) && (strucboucur < FWBNEXT)) {
		...
		if((feasyllabiccur IS_MINUS) && ((struccur & FWINITC) IS_MINUS)) {
			if(((feacur & FOBST) IS_PLUS) && ((feacur & FPLOSV) IS_MINUS) && ((struccur & FBOUNDARY) >= FWBNEXT))
		...
		/* Penultimate lengthening of stressed syllabic if hat fall f0 gesture */
			...
			if(feasyllabiccur IS_PLUS) {
				if(((struccur & FHAT_ENDS) IS_PLUS) && (strucboucur < FVPNEXT) && (strucboucur > FMBNEXT)) {
					deldur = deldur + NF25MS;
				}
			}
		}
		/* Rule 8: Lengthen each seg of an emphasized syllable, including rime */
		if(((struccur & FWINITC) IS_PLUS) || ((feasyllabiccur IS_PLUS) && (strucstresscur != FEMPHASIS))) {
			emphasissw = FALSE;
		}
		...
		/* Does rule apply ? */
		if((feasyllabiccur IS_PLUS) || ((phocur >= UKP_LX) && (phocur <= UKP_NX) && ((struccur & (FSTRESS | FWINITC)) IS_MINUS) && ((pDphsettar->feanex & FOBST) IS_PLUS))) {
			/* Determine whether next segment is postvocalic consonant */
			if(((pDphsettar->feanex & FSYLL) IS_MINUS) && ((pDphsettar->strucnex & (FSTRESS | FWINITC)) IS_MINUS)) {
				posvoc = pDphsettar->phonex_timing;
				/* See if postvocalic consonant is a sonorant */
				/* or if postvoc sonor is followed by an obst cons */
				if(((posvoc >= UKP_LX) && (posvoc <= UKP_NX)) && ((phone_feature(pDph_t, pDph_t->allophons[nphon + 2]) & FOBST) IS_PLUS) && ((pDph_t->allofeats[nphon + 2] & (FSTRESS | FWINITC)) IS_MINUS)) {
					psonsw = 1;
				...
				if(posvoc != GEN_SIL) {
					if((phone_feature(pDph_t, posvoc) & FVOICD) IS_MINUS) {
						...
						arg1 = N80PRCNT;
						if(((phone_feature(pDph_t, posvoc) & FPLOSV) IS_PLUS) || (posvoc == UKP_CH)) {
		  ...
		  // was that it was redundant to rule 4 and was coded incorrectly
					...
					/* Postvocalic segment is voiced */
						...
						if((phone_feature(pDph_t, posvoc) & FOBST) IS_PLUS && phocur != UKP_EN) {
							...
							if(((phone_feature(pDph_t, posvoc) & FPLOSV) IS_MINUS)
							   ...
							   && (posvoc != UKP_DX) && ((feacur & FSYLL) IS_PLUS)) {
								deldur = deldur + NF25MS;
							}
			...
			if(((phocur == UKP_N) && (pDphsettar->phonex_timing == UKP_T)) && ((pDphsettar->strucnex & (FWINITC | FSTRESS)) IS_MINUS)) {
				arg1 = N10PRCNT;
				if(((phone_feature(pDph_t, pDph_t->allophons[nphon + 2]) & FSYLL) IS_PLUS) && ((pDph_t->allofeats[nphon + 2] & FMEDIALSYL) IS_MINUS)) {
		...
		if(feasyllabiccur IS_PLUS) {
			...
			if(((struccur & FTYPESYL) == FFIRSTSYL) && ((struccur & FSTRESS_1) IS_PLUS) && ((struclas & FWINITC) IS_MINUS)) {
				deldur += NF25MS;
			}
		...
		/* Rule 13: Shorten consonant clusters */
			...
			if((feacur & FCONSON) IS_PLUS) {
				if(((pDphsettar->feanex & FCONSON) IS_PLUS) && (strucboucur < FVPNEXT)) {
					/* First consonant of a two - consonant sequence */
					/* Default shortening is 70 percent */
					// unless plosive plosive sequence
					if((pDphsettar->feanex & FPLOSV) IS_MINUS || (feacur & FPLOSV) IS_MINUS)
						arg1 = N70PRCNT;
					/* Length nasal by 1.5 if next cons is word-init */
					if(((feacur & FNASAL) IS_PLUS) && ((pDphsettar->strucnex & FWINITC) IS_PLUS)) {
						// from 1.5 to 1.2
						arg1 = N120PRCNT;
					}
				...
				if(((fealas & FCONSON) IS_PLUS) && ((struclas & FBOUNDARY) < FVPNEXT)) {
		...
		if((feacur & FSON1) IS_PLUS) {
			if(((fealas & FVOICD) IS_MINUS) && ((fealas & FPLOSV) IS_PLUS)) {
				deldur = deldur + NF20MS;
			}
		}
		...
		if((feacur & FVOWEL) IS_PLUS) {
			if(((fealas & FSON2) IS_PLUS) && ((fealas & FNASAL) IS_MINUS)) {
				if(deldur == 0)
					deldur = NF20MS;
			}
		}
		...
		if(((feacur & FPLOSV) IS_PLUS) && (feacur & FOBST)) {
			arg1 = N80PRCNT;
		...
		if(((fealas & FVOICD) IS_PLUS && (feacur & FVOICD) IS_MINUS))
		...
		if(phone_feature(pDph_t, pDph_t->allophons[nphon]) & FOBST && phone_feature(pDph_t, pDph_t->allophons[nphon]) & FSTOP && phone_feature(pDph_t, pDph_t->allophons[nphon]) & FPLOSV && pDph_t->allophons[nphon + 1] != UKP_DZ) {
			...
			if(phone_feature(pDph_t, pDph_t->allophons[nphon + 1]) & FVOICD) {
				...
				if(phone_feature(pDph_t, pDph_t->allophons[nphon + 1]) & FCONSON && phone_feature(pDph_t, pDph_t->allophons[nphon + 1]) & FSTOP && phone_feature(pDph_t, pDph_t->allophons[nphon + 1]) & FOBST) {
					if(durmin <= 6)
						durmin = 6;
				} else {
					if(durmin <= 14)
		...
		if(((struccur & FSTRESS) IS_PLUS))
			...
			/* 0015 EAB     06/11/97		Added in Fasttalk time alignement feature */
				...
				for(endcnt = nphon - 1; stcnt - (endcnt); endcnt--) {
					...
					if((phone_feature(pDph_t, pDph_t->allophons[endcnt]) & FSON1) && !(phone_feature(pDph_t, pDph_t->allophons[endcnt]) & FNASAL))

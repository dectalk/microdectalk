...
#define Fact_Elongation PCent6
	...
	for(nphon = 0; nphon < pDph_t->nallotot; nphon++) {
		if(nphon - 3 >= 0) SegG3 = pDph_t->allophons[nphon - 3];
		else
			SegG3 = GEN_SIL;
		if(nphon - 2 >= 0) SegG2 = pDph_t->allophons[nphon - 2];
		else
			SegG2 = GEN_SIL;
		if(nphon - 1 >= 0) pholas = pDph_t->allophons[nphon - 1];
		else
			pholas = GEN_SIL;
		...
		if(nphon + 2 < pDph_t->nallotot) SegD2 = pDph_t->allophons[nphon + 2];
		else
			SegD2 = GEN_SIL;
		if(nphon + 3 < pDph_t->nallotot) SegD3 = pDph_t->allophons[nphon + 3];
		else
			SegD3 = GEN_SIL;
		...
		if(pDph_t->user_durs[nphon] != 0) {
			...
			case F_Z:
				...
				case F_K:
					switch(pDphsettar->phonex_timing & PVALUE) {
					case F_S:
					case F_T: // FI[K]SE1, VI[K]TOR
						pcnt = mlsh1(pcnt, N70PRCNT);
						regle(phocur, "[K].(S,T)  0.70");
						break;
					case F_AN:
					case F_O: // [K]AnPAGn, [K]OM, In[K]ONU
						pcnt = mlsh1(pcnt, N120PRCNT);

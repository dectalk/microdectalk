...
#else
	F1inHZ = FormantScale * (FLTPNT_T)variabpars[OUT_F1] + (256.0 - 256.0 * FormantScale);

	F2inHZ = FormantScale * (FLTPNT_T)variabpars[OUT_F2] + (512.0 - 512.0 * FormantScale);
	...
	for(uiNs = 0; uiNs < pVtm_t->uiNumberOfSamplesPerFrame; uiNs++) {
		...
		for(nsr4 = 0; nsr4 < 4; nsr4++) {
			...
			pVtm_t->T0 = T0inS4 >> 2;
				...
				iTemp = pVtm_t->T0Jitter * pVtm_t->T0;
				; /*  Add jitter, if any.              */
				pVtm_t->T0 += iTemp;
...
#ifdef HLSYN
	pVtm_t->PartialCascadePathGain = (FLTPNT_T)32264.0 * c1_gain * c2_gain * c3_gain * pVtm_t->c4_b0 * pVtm_t->c5_b0 * pVtm_t->NasalResonator_b0;
#else
	pVtm_t->PartialCascadePathGain = (FLTPNT_T)11264.0 * c1_gain * c2_gain * c3_gain * pVtm_t->c4_b0 * pVtm_t->c5_b0 * pVtm_t->NasalResonator_b0;
#endif
...
setR1(Fx, Bx) int Fx; /* Desired F1 value */

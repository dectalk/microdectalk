	...
	for(n = 0; n < pDph_t->nphonetot; n++) {
		...
		case F_NG:
			if(s_SegDroite & FSYNT) {
				...
				if((!(Traits_Temp & FVOICD) || Traits_Temp & FNASAL) && !(phone_feature(pDph_t, SegDroite) & FVOWEL)) {
					// KOuP[L] DE ..., SAK[R] DE
					IAp = F_AP;
				}
			}
...
void phalloph(LPTTS_HANDLE_T phTTS) {
	...
	for(n = 0; n < pDph_t->nphonetot; n++) {
		...
		if(((phone_feature(pDph_t, pDph_t->phonemes[n + 1]) & FSYLL) IS_PLUS) && (curr_inph == USP_AX) && ((curr_instruc & FBOUNDARY) IS_PLUS) && (pDph_t->phonemes[n - 1] == USP_DH) && ((pDph_t->sentstruc[n - 1] & FWINITC) IS_PLUS)) {
			curr_outph = USP_IY;
		}
		...
		if((curr_inph == USP_AX) && (next_inph == GEN_SIL) && n == 1 && (Cite_It)) {
			curr_outph = USP_EY;
		}
		// #ifndef NWSNOAA
		/* Rule 1b:  Unreduce vowel in "for" if following segment is vowel or sil */
		if((curr_inph == USP_F) && (next_inph == USP_RR) && ((((pDph_t->sentstruc[n + 1] & FSTRESS) IS_MINUS) && ((pDph_t->sentstruc[n + 1] & FTYPESYL) == FMONOSYL)) || Cite_It)) {
			if(((phone_feature(pDph_t, pDph_t->phonemes[n + 2]) & FSYLL) IS_PLUS) || (pDph_t->phonemes[n + 2] == GEN_SIL)) {
		...
		if((curr_inph == GEN_SIL) && (pDph_t->phonemes[n + 1] == USP_EH) && (pDph_t->phonemes[n + 2] == USP_T) && Cite_It) {
#ifdef DEBUG_OLDALLO
		...
		if(((curr_instruc & (FSTRESS | FWINITC)) IS_MINUS) && ((phone_feature(pDph_t, pDph_t->phonemes[n - 1]) & FVOWEL) IS_PLUS)) {
			if(curr_inph == USP_LL)
				curr_outph = USP_LX;
			...
			if(curr_inph == USP_R) {
				...
				if((symlas == USP_EY) || (symlas == USP_EH) || (symlas == USP_AE)) {
		...
		if(((next_inph == USP_YU) || (next_inph == USP_YX)) && ((pDph_t->sentstruc[n + 1] & FSTRESS) IS_MINUS)) {
			if(curr_inph == USP_T) {
				curr_outph = USP_CH;
				goto endrul3;
			}
		...
		if(curr_inph == USP_T) {
			if((next_inph == USP_LL) || (next_inph == USP_DH) || ((((curr_instruc & FBOUNDARY) >= FMBNEXT) && (((phone_feature(pDph_t, next_inph) & FSON2) IS_PLUS) || (next_inph == USP_HX))) || (next_inph == USP_EN))) {
				curr_outph = USP_D;
		...
		if(((curr_inph == USP_D) || (curr_inph == USP_T)) && ((curr_instruc & FSTRESS) IS_MINUS)) {
			/* Consonant must be preceded by vowel,rx,en,G,el */
			if(((phone_feature(pDph_t, last_outph) & FSON1) IS_PLUS) && (last_outph != USP_M) && (last_outph != USP_NX) && ((last_outph != USP_N))
			   ...
			   && ((phone_feature(pDph_t, next_inph) & FSYLL) IS_PLUS)) {
				 ...
				 * [ow]
				 */
				else if((((pDph_t->allofeats[pDph_t->nallotot - 1] & FSTRESS) IS_PLUS) && (next_inph == USP_OW)) || (next_inph == USP_AX) || (next_inph == USP_RR) || (next_inph == USP_IY) || (next_inph == USP_IX) || (next_inph == USP_EL)) {
					if(curr_inph == USP_T) {
						curr_outph = USP_DF;
		...
		if((curr_inph == USP_DH) && ((curr_instruc & FSTRESS) IS_MINUS)) {
		...
		if((curr_inph == HX) && ((phone_feature(pDph_t, last_outph) & FVOICD) IS_PLUS) && ((curr_instruc & FSTRESS) IS_MINUS)) {
			delete_short = TRUE;
		}
		/* eab latin mode rule */
...
#ifdef ENGLISH_UK
		/* Rule 1a: The word "the" should be /dh iy/ before a syllabic */
		if(((phone_feature(pDph_t, pDph_t->phonemes[n + 1]) & FSYLL) IS_PLUS) && (curr_inph == AX) && ((curr_instruc & FBOUNDARY) IS_PLUS) && (pDph_t->phonemes[n - 1] == DH) && ((pDph_t->sentstruc[n - 1] & FWINITC) IS_PLUS)) {
			curr_outph = IY;
		}
		/* rule 1c make a long a be ah */
		Cite_It = 0;
		...
		if(pKsd_t->sprate < 100)
			Cite_It = TRUE;
		if((curr_inph == AX) && (next_inph == GEN_SIL) && n == 1 && (Cite_It)) {
			curr_outph = EY;
		}

		/* Rule 1b:  Unreduce vowel in "for" if following segment is vowel or sil */

		if((curr_inph == F) && (next_inph == AX) && ((((pDph_t->sentstruc[n + 1] & FSTRESS) IS_MINUS) && ((pDph_t->sentstruc[n + 1] & FTYPESYL) == FMONOSYL)) || Cite_It)) {
			if(((phone_feature(pDph_t, pDph_t->phonemes[n + 2]) & FSYLL) IS_PLUS) || (pDph_t->phonemes[n + 2] == GEN_SIL)) {
			...
			if(pDph_t->phonemes[n + 1] == AX)
				if(pDph_t->phonemes[n + 2] == N)
					if(pDph_t->phonemes[n + 3] == D)
						if((((pDph_t->sentstruc[n + 1] & FSTRESS) IS_MINUS) && ((pDph_t->sentstruc[n + 3] & FSTRESS) IS_MINUS)) || Cite_It) {
#ifdef DEBUG_OLDALLO
							printf("and eh->ae rule firing");
#endif
		...
		if((curr_inph == T) && (next_inph == AX) && ((((pDph_t->sentstruc[n + 1] & FSTRESS) IS_MINUS) && ((pDph_t->sentstruc[n + 1] & FTYPESYL) == FMONOSYL)) || Cite_It)) {
			if(((phone_feature(pDph_t, pDph_t->phonemes[n + 2]) & FSYLL) IS_PLUS) || (pDph_t->phonemes[n + 2] == GEN_SIL)) {
		...
		/* Rule 1c:  Unreduce vowel in citation mode for  "at" to be [ae] */
		if((curr_inph == GEN_SIL) && (pDph_t->phonemes[n + 1] == AX) && (pDph_t->phonemes[n + 2] == T) && Cite_It) {
#ifdef DEBUG_OLDALLO
			printf("at eh->ae rule firing");
#endif
		...
		/* Palatalize /t/ or /d/ if next phoneme is unstressed /y/ */
		if(((next_inph == YU) || (next_inph == Y)) && ((pDph_t->sentstruc[n + 1] & FSTRESS) IS_MINUS)) {
			if(curr_inph == T) {
				curr_outph = CH;
				goto endrul3;
			}
		...
		if(curr_inph == T) {
			if((next_inph == EL) && ((((curr_instruc & FBOUNDARY) >= FMBNEXT) && (((phone_feature(pDph_t, next_inph) & FSON2) IS_PLUS))) || (next_inph == EN))) {
		...
		if((curr_inph == DH) && ((curr_instruc & FSTRESS) IS_MINUS)) {
			if((last_outph == T) || (last_outph == TX) || (last_outph == D)) {
				curr_outph = DZ;
			}
			if(last_outph == N) {
				curr_outph = N;
	...
	endrul3:
		...
		if((phone_feature(pDph_t, curr_inph) & (FVOWEL | FSEMIV)) IS_MINUS || (phone_feature(pDph_t, last_outph) & (FVOWEL | FSEMIV)) IS_MINUS)
		 ...
		 */
		else if((curr_outstruc & FSTRESS) IS_PLUS && (pDph_t->allofeats[pDph_t->nallotot - 1] & FBOUNDARY) >= FWBNEXT && (pDph_t->allofeats[pDph_t->nallotot - 2] & FBOUNDARY) < FWBNEXT) {
		} else if((pDph_t->allofeats[pDph_t->nallotot - 1] & FSTRESS) IS_MINUS && (phone_feature(pDph_t, last_outph) & FDIPTH) IS_PLUS) {
			/*
		...
		} else if((curr_outstruc & FSTRESS) IS_MINUS && (phone_feature(pDph_t, curr_inph) & FDIPTH) IS_PLUS) {
			/*
			 * First, check whether we should defer dipthongization
			 * to the following segment: "uno u otro" -> ['uno wx'otro]
			 */
			if((phone_feature(pDph_t, next_inph) & (FVOWEL | FDIPTH)) == FVOWEL && (pDph_t->sentstruc[n + 1] & FSBOUND) IS_MINUS) {
				goto nodipth;
			}
			/*
	...
	nodipth:;
		...
		/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
#endif
		if((pDph_t->f0mode == NORMAL) && ((phone_feature(pDph_t, curr_inph) & FSYLL) IS_PLUS) && ((curr_instruc & FSTRESS) IS_PLUS) /* 1-str 2-str emph */ /* S1 S2 S3 SENPH in GERMAN */
		   && (emphasislock == FALSE)) {
			 ...
			 */
			if((hatposition == AT_TOP_OF_HAT) && ((curr_instruc & FSTRESS_1) IS_PLUS)) { /* 1-str emph */
...
/*				tmp = remaining_stresses_til (pDph_t,n, FCBNEXT);            */ /* useless value is never used */
				...
				if((remaining_stresses_til(pDph_t, n, FCBNEXT) == 0)
				   ...
				   || doitlater
					...
					if(((curr_instruc & FBOUNDARY) == FVPNEXT) && (promote_last_2(pDph_t, n))) {
...
/* doitlater suffers from the problem of unknown deletes so you don't really know how far from the end you really are*/
				...
				if((stresses_in_phrase > 1) && (remaining_stresses_til(pDph_t, n, FVPNEXT) == 0) && (remaining_stresses_til(pDph_t, n, FCBNEXT) > 1)) {
					curr_outstruc |= FHAT_ENDS;
					hatposition	   = AT_BOTTOM_OF_HAT;
					stresses_in_phrase = 0;
				}
...
static short remaining_stresses_til(PDPH_T pDph_t, short msym, short b_type) {
	...
	for(m = msym; m < pDph_t->nphonetot; m++) {
		...
		if((m != msym) && ((pDph_t->sentstruc[m] & FSTRESS_1) IS_PLUS) && ((phone_feature(pDph_t, pDph_t->phonemes[m]) & FSYLL) IS_PLUS)) {
			count++;
		}
		if(((pDph_t->sentstruc[m] & FBOUNDARY) >= b_type) || (pDph_t->phonemes[m] == GEN_SIL)) {
			return (count);
		}
	}
	return (count);
	...
	for(m = msym; m < pDph_t->nphonetot; m++) {
		...
		if((m != msym) && ((pDph_t->sentstruc[m] & FSTRESS) IS_PLUS) && ((phone_feature(pDph_t, pDph_t->phonemes[m]) & FSYLL) IS_PLUS)) {
			count++;
		}
		if(((pDph_t->sentstruc[m] & FBOUNDARY) >= b_type) || (pDph_t->phonemes[m] == GEN_SIL)) {
			return (count);
...
static short promote_last_2(PDPH_T pDph_t, short msym) {
	...
	for(m = msym; m < pDph_t->nphonetot; m++) {
		...
		if(((pDph_t->sentstruc[m] & FBOUNDARY) >= FCBNEXT) || (pDph_t->phonemes[m] == GEN_SIL)) {
			if(done_it != 0) {

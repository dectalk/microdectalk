...
loop:
...
#endif
	{
		...
		if((ptp[0] & P2SYL) != 0	 /* If 2 syl. prefix and	*/
		   && csyl < pLts_t->nsyl - 1) { /* some root remains.	*/
	...
	if(pp1 != lpp /* V-C1-C2 case.	*/
	   && ls_adju_is_voc(pp1) != FALSE && (pp1 = pp1->p_fp) != lpp && ls_adju_is_cons(pp1) != FALSE && (pp2 = pp1->p_fp) != lpp && (pp2->p_flag & PFSYLAB) != 0 && pp1->p_sphone == pp2->p_sphone) {
		pLts_t->sylp[csyl]->p_flag |= PFRFUSE;
	}
#endif
	return (TRUE);
...
void ls_adju_best2syl(PLTS_T pLts_t) {
	...
	if(ls_adju_is_voc(pp) != FALSE && ls_adju_is_cons(pp->p_fp) != FALSE && ls_adju_is_cons(pp->p_fp->p_fp) != FALSE) {
#ifdef ENGLISH_US
...
void ls_adju_final_fixes(PLTS_T pLts_t) {
	...
	if(pp->p_sphone == US_AX && (pp = pp->p_fp) != &pLts_t->phead && pp->p_sphone == US_N && (pp = pp->p_fp) != &pLts_t->phead && (pp->p_flag & PFSYLAB) != 0) {
		pp	     = pLts_t->phead.p_fp;
		pp->p_sphone = US_AH;
		pp->p_uphone = SIL;
		pp->p_stress = SPRI;
	}
	if(pLts_t->nsyl == 3) { /* "camera" rule.	*/
...
#ifdef ENGLISH_UK
	pp = pLts_t->phead.p_fp; /* [xn] => [^n].	*/
	if(pp->p_sphone == UK_AX && (pp = pp->p_fp) != &pLts_t->phead && pp->p_sphone == UK_N && (pp = pp->p_fp) != &pLts_t->phead && (pp->p_flag & PFSYLAB) != 0) {
		pp	     = pLts_t->phead.p_fp;
		pp->p_sphone = UK_AH;
		pp->p_uphone = SIL;
		pp->p_stress = SPRI;

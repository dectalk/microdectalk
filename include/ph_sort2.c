...
static void init_med_final(LPTTS_HANDLE_T phTTS, short msym) {
	...
	for(m = msym + 1; m < pDph_t->nsymbtot; m++) {
		if((pDph_t->symbols[m] >= WBOUND) && (pDph_t->symbols[m] <= EXCLAIM)) { /* Look for next boundary */
...
static void get_next_bound_type(LPTTS_HANDLE_T phTTS, short msym) {
	...
	for(m = msym + 1; m < pDph_t->nsymbtot; m++) {
		if(((pDph_t->symbols[m] & PVALUE) >= SBOUND) && ((pDph_t->symbols[m] & PVALUE) <= EXCLAIM)) { /* Look for next boundary */

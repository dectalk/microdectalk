...
void pht0draw(LPTTS_HANDLE_T phTTS) {
	...
	short	      f0seg = 0, f0in = 0; /*,pDphsettar->f0command=0*/
	;				   /*MVP : Made as local variable*/
...
/*	  Set Beginning and final frequency of baseline fall */
		...
		while((pDphsettar->nfram >= pDphsettar->dtimf0) &&
			...
			if(pDph_t->f0mode == TIME_VALUE_SPECIFIED) {
				...
				case STEP:
					...
					if(pDphsettar->f0command < 0) { /* Cancel previous impulse if step */
									/* is of opposite sign */
		...
		9/19/97 */
		if(pDphsettar->nframb > pDph_t->tcumdur - 12 && pDph_t->glotalize && pDph_t->number_words > 3) {
...
/* eab 11/22/97 per caroline remove glotalization */
		...
		if(pDph_t->f0mode < SINGING) {
			...
			} else if(pDphsettar->glide_inc < 0) {
				if(pDphsettar->glide_step >= pDphsettar->glide_inc) {
					pDphsettar->glide_inc = 0;
				}
			}
...
/*	  Set Beginning and final frequency of baseline fall */
		...
		/*EAB Drop pitch rapidly at very end of utterance
		9/19/97 */
		if(pDphsettar->nframb > pDph_t->tcumdur - 14 && pDph_t->glotalize && pDph_t->number_words > 3) {
...

{
	...
	if(pDphsettar->nframg >= pDphsettar->segdrg) {
		...
		if(((phone_feature(pDph_t, pDph_t->allophons[pDphsettar->npg + 1]) & FVOWEL) IS_PLUS) && ((pDph_t->allofeats[pDphsettar->npg + 1] & (FMEDIALSYL & FFINALSYL)) IS_MINUS) && ((pDph_t->allofeats[pDphsettar->npg] & FBOUNDARY) >= FWBNEXT)
		   /* && (pDph_t->allophons[pDphsettar->npg+1] != YU)*/) {
			...
			if((phone_feature(pDph_t, pDph_t->allophons[pDphsettar->npg]) & FSYLL) IS_PLUS) {
				if(((pDph_t->allophons[pDphsettar->npg] == pDph_t->allophons[pDphsettar->npg + 1]) && ((pDph_t->allofeats[pDphsettar->npg + 1] & FSTRESS_1) IS_PLUS)) || ((pDph_t->allofeats[pDphsettar->npg] & FBOUNDARY) >= FVPNEXT)) {
		...
		if((pDph_t->allophons[pDphsettar->npg] == GRP_RR) && ((phone_feature(pDph_t, pDph_t->allophons[pDphsettar->npg + 1]) & FVOWEL) IS_PLUS)) {
			pDphsettar->tglstn = pDphsettar->segdrg;
		}
#endif
...
void pht0draw(LPTTS_HANDLE_T phTTS) {
	...
	if(pDph_t->nf0ev <= -2) { /* Hard initialization */
#ifdef GERMAN
		/* @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ */
		/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
...
/* Segmental effect on f0, reset tarseg if time exceeds cum dur */
	...
	if(pDph_t->f0mode < SINGING) {
		...
		pseudojitter = getcosine[pDphsettar->timecos15 >> 6] + getcosine[pDphsettar->timecos10 >> 6];
#ifdef MSDBG
...
static void filter_commands(PDPH_T pDph_t, short f0in) { /* Convert 'f0in' command to smoothed 'f0' */
...
static void linear_interp(PDPH_T pDph_t) {
	...
	if(pDphsettar->delnote >= 0) { /* Do not overshoot newnote */
		if(pDph_t->f0 > pDphsettar->newnote) {
			pDph_t->f0	    = pDphsettar->newnote;
			pDphsettar->f0start = pDphsettar->newnote;
			pDphsettar->delcum  = 0;

...
LETTER* ls_task_parse_number(LPTTS_HANDLE_T phTTS, LETTER* llp, LETTER* rlp, NUM* np) {
	...
	if(tlp1 != rlp && tlp1->l_ch == pLts_t->fchar) { /* Fraction             */
		np->n_flp = tlp1;
		++tlp1;
	...
	if(tlp1 != rlp && tlp1->l_ch == 'e' && (pKsd_t->modeflag & MODE_MATH)) { /* Exponant.            */
		tlp2 = tlp1;
		++tlp1;
...
void ls_task_readword(LPTTS_HANDLE_T phTTS, LETTER word[]) {
	 ...
	 */
	while(ls_util_is_white(&(pLts_t->citem)) != FALSE) { /* Skip spaces.         */
		ls_util_read_item(phTTS);
	...
	while(pLts_t->cur_read_pos <= pLts_t->cur_input_pos) {
		...
		if(lp1 < lp2) { /* Overstrike.          */
			...
			if(lp1 == &word[0]) { /* Initial position.    */
				if(t == PHONEME) {
					ls_util_write_item(phTTS);
					ls_util_read_item(phTTS);
...
int ls_task_minidic_search(LPTTS_HANDLE_T phTTS, LETTER* llp, LETTER* rlp) {
	...
	if((cp = (char*)ls_task_wlookup(pLts_t, llp, (unsigned char*)&sdic[0])) != NULL) { /*eab mini dictionary lookup */
...
//	PKSD_T pKsd_t = phTTS->pKernelShareData;
	...
	} else {										     // CAB 08/15/2002 removed warnings by typecast
...
will get more eligant with the re-write*/
	...
	if((cp = (char*)ls_task_prelookup(pLts_t, llp, &count, &predic[0])) != NULL) { /*eab mini dictionary lookup */
		ls_util_send_phone_list(phTTS, cp);
		pLts_t->lbphone = WBOUND;
		// pLts_t->rbphone = WBOUND;
		// ls_util_send_phone(phTTS,IX);
...
int ls_task_dictionary_after_punct(LPTTS_HANDLE_T phTTS, LETTER* llp, LETTER** rlp) {
	...
	if(((pLts_t->lflag) & (LSTRIP | RSTRIP)) != 0 && (flag = ls_util_lookup(phTTS, llp, (*rlp), context)) != MISS) {
...
int ls_task_single_letter_spell_rules(LPTTS_HANDLE_T phTTS, LETTER* llp, LETTER* rlp) {
	...
	if(llp + 1 == rlp) {
		...
		if(IS_UPPER(c) && ((pLts_t->lflag) & (LSTRIP | RSTRIP)) == 0 && ls_util_is_dot(pLts_t) != FALSE) {
		 ...
		 */
		if((c == 'a' || c == 'A') && (pLts_t->isnumabr == 0 || pLts_t->isnumabr == 1) && ((pLts_t->lflag) & (LSTRIP | RSTRIP)) == 0 && ls_util_is_white(&(pLts_t->citem)) == TRUE) {
#ifdef NEW_LTS
...
#ifdef SPANISH_SP
		/*igrega or i code*/
		if((c == 'y' || c == 'Y') && ((pLts_t->lflag) & (LSTRIP | RSTRIP)) == 0 && ls_util_is_white(&(pLts_t->citem)) != FALSE) {
...
#else
		if(c != 0xBC && c != 0xBD && ((lsctype[c] & O) == 0) && !IS_DIGIT(llp->l_ch))
#endif // FRENCH
		{
#ifdef LS1DEBUG_OLD
...
int ls_task_Dr_St_process(LPTTS_HANDLE_T phTTS, LETTER* llp, LETTER* rlp) {
	...
	if((llp + 3 == rlp && (llp + 2)->l_ch == '.') || (llp + 2 == rlp && ((pLts_t->lflag) & RSTRIP) == 0 && ls_util_is_dot(pLts_t) != FALSE)) {
		c1 = (llp + 0)->l_ch;
		c2 = (llp + 1)->l_ch;
...
#endif
{
	...
	if(llp->l_ch == '$' || llp->l_ch == 0xA3 || llp->l_ch == 0x80) {
		...
		if(llp->l_ch == '-'			       /* Signs.               */
		   || llp->l_ch == '+' || llp->l_ch == 0xB1) { /* Plus-minus.          */
			...
			if(ls_util_is_dot(pLts_t) == FALSE /* Check if special.    */
				...
				*/
				...
				if(llp == rlp - 1 /* one digit */
				   || ((rlp - 1)->l_ch != '0' && (rlp - 2)->l_ch != '1' && (rlp - 2)->l_ch != '7' && (rlp - 2)->l_ch != '9')) {
					/*
					  1 digit : 1 enfant
					  not : ..0    : 70, 100
					  not 1. 7. 9. : 12, 17, 99
					...
					*/
					...
					switch((rlp - 1)->l_ch) {
					case '1':
						strcpy(pLts_t->precedent, "un");
						break;
					case '2':
						strcpy(pLts_t->precedent, "deux");
						break;
					case '3':
						strcpy(pLts_t->precedent, "trois");
						break;
					case '4':
						strcpy(pLts_t->precedent, "quatre");
						break;
					case '5':
						strcpy(pLts_t->precedent, "cinq");
						break;
					case '6':
						strcpy(pLts_t->precedent, "six");
						break;
					case '7':
						strcpy(pLts_t->precedent, "sept");
						break;
					case '8':
						strcpy(pLts_t->precedent, "huit");
						break;
					case '9':
						strcpy(pLts_t->precedent, "neuf");
						break;
					}
					...
					case '7':
					case '9':
						strcpy(pLts_t->precedent, "dis");
						break;
					case '2':
						strcpy(pLts_t->precedent, "vingt");
						break;
					case '8':
						strcpy(pLts_t->precedent, "vingts");
						break;
					}
...
#else
				if(lp2 + 3 == rlp && lp2->l_ch == (unsigned char)'è' && (lp2 + 1)->l_ch == 'm' && ((lp2 + 2)->l_ch == 'e')) {
					...
					return (2);
					  ...
					  && (llp + 1)->l_ch == (unsigned char)'è' && (llp + 2)->l_ch == 'r' && (llp + 3)->l_ch == 'e') {
...
int ls_task_part_number(LPTTS_HANDLE_T phTTS, LETTER* llp, LETTER* rlp) {
	...
	if(((pLts_t->lflag) & DIGSLSH) != 0) {	    /* Parts have digits or */
...
/* JDB: language dependent */
		...
		if(lp2 != lp1) {
			...
			if(((pLts_t->lflag) & HCONS) == 0) { /* If no consonant, the */
							     /* if (lp2+1 != lp1)
							     { */
#ifdef LS1DEBUG_OLD
		...
		*/
		...
		while(lp1 != rlp) { /* 3 loops hiv-cv-dc */
			...
			if((lflag & HVOWEL) == 0) {
				...
				// CAB 08/15/2002 Removed warnings by typecast
				if(lp2 + 1 == lp1 && lp1->l_ch == '-' && ls_task_wlookup(pLts_t, lp2, (unsigned char*)apres_tiret)) {
...
void ls_task_parse_sentence(LPTTS_HANDLE_T phTTS) {
	...
	for(i = 0; i < pLts_t->cur_input_pos; i++) {
		for(j = i; (j < pLts_t->cur_input_pos &&
			    (char_types[pLts_t->input_array[j]] & MARK_space));
		    j++, i++);

		k = 0;
		for(j = i; (j < pLts_t->cur_input_pos &&
			    (!(char_types[pLts_t->input_array[j]] & MARK_space)) && k < 200);
		    j++, k++, i++) {

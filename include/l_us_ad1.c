...
int ls_adju_cluster(int f, int s) {
	...
	case US_S:
		if(s == US_W || s == US_LL || s == US_P || s == US_T || s == US_K || s == US_M || s == US_N || s == US_F)
			return (OK);
		break;

	case US_SH:
		if(s == US_W || s == US_LL || s == US_R || s == US_P || s == US_T || s == US_M || s == US_N)
			return (OK);
	} /* 	switch (f) */
	return (ILLEGAL);
}
...
void ls_adju_allo2(PLTS_T pLts_t) {
	...
	if(bflag == FALSE) {
		...
		while(pp1 != &pLts_t->phead) {
			...
			if((fthis & PFBLOCK) == 0 && (pp1->p_uphone != SIL && sthis == SUN)) {
	...
	while(pp1 != &pLts_t->phead) {
		...
		if(sthis == SUN && (ph1 == US_AX || ph1 == US_IX)) {
			pp2 = pp1->p_fp;
			if(pp2->p_sphone == US_LL && (pp2->p_flag & PFSYLAB) == 0 && (pp2->p_fp->p_flag & PFMORPH) != 0) {
		...
		if(sthis != SUN && (ph1 == US_LL || ph1 == US_R) && ls_adju_is_obs(pp1->p_bp) != FALSE && (pp1->p_fp->p_flag & PFMORPH) != 0) {
			pp1->p_sphone = US_RR;
			if(ph1 == US_LL)
				pp1->p_sphone = US_EL;
			pp1 = pp1->p_fp;
		 ...
		 * [x][r][=][r] => [x][=][r]	(arrange)
		 */
		if(ph1 == US_AX && (pp2 = pp1->p_fp)->p_sphone == US_R && (pp2->p_flag & PFSYLAB) == 0) {
			ls_adju_del_phone(pLts_t, pp2);
		 ...
		 * [n] => [G] / - {[k],[g]}
		 */
		if(ph1 == US_N && ((pp2 = pp1->p_fp)->p_sphone == US_K || pp2->p_sphone == US_G) && (pp2->p_flag & PFSYLAB) == 0) {
			pp1->p_sphone = US_NX;
			pp1	      = pp1->p_fp;
			continue;
		}
		...
		if(ph1 == US_G || ph1 == US_K) {
			if(ph1 == US_G) /* [g] => [J]		*/
				...
				ph1 = US_S;
			pp2 = pp1->p_fp;
			if(pp2->p_sphone == US_AY && (pp2 = pp2->p_fp)->p_sphone == US_Z && (pp2->p_fp->p_flag & PFMORPH) != 0) {
				pp1->p_sphone = ph1;
				pp1	      = pp1->p_fp;
				continue;
			} /*if (pp2->p_sphone == AY */
				...
				if(pp2->p_sphone == US_IX) {
					pp2 = pp2->p_fp;
					if(pp2->p_sphone == US_D && (pp2->p_fp->p_flag & PFMORPH) != 0) {
						pp1->p_sphone = ph1;
						pp1	      = pp1->p_fp;
						continue;
					}
		...
		if(ph1 == US_D) {
			 ...
			 * [d] => [J] / - [u][L] +
			 */
			if((pp2 = pp1->p_fp)->p_sphone == US_UW && ((pp2 = pp2->p_fp)->p_sphone == US_LL || pp2->p_sphone == US_EL) && (pp2->p_fp->p_flag & PFMORPH) != 0) {
				pp1->p_sphone = US_JH;
				pp1	      = pp1->p_fp;
				continue;
			}
		...
		if(ph1 == US_S) {
			 ...
			 * [k][s] => [g][z] / [|] - [+Voc,+Stress]
			 */
			if((pp2 = pp1->p_bp)->p_sphone == US_K && pp2->p_bp->p_sphone == US_IX && sleft == SUN && ls_adju_is_voc(pp1->p_fp) != FALSE && sthis != SUN) {
				pp2->p_sphone = US_G;
				pp1->p_sphone = US_Z;
				continue;
			}
		...
		if(ph1 == US_T) {
			...
			if(pp2->p_sphone == US_UW) {
				...
				if(pp2->p_sphone == US_IY) {
					pp3 = pp2->p_fp;
					if(pp3->p_sphone == US_AX && (pp3->p_fp->p_flag & PFMORPH) != 0) {
						if((pp2->p_flag & PFSYLAB) != 0)
							return;
						ls_adju_del_phone(pLts_t, pp2);
						pp1->p_sphone = US_SH;

...
void speech_waveform_generator(LPTTS_HANDLE_T phTTS) {
	...
	for(ns = 0; ns < pVtm_t->uiNumberOfSamplesPerFrame; ns++) {
		...
		voice	      = frac1mul(pVtm_t->one_minus_decay, voice) + frac1mul(pVtm_t->decay, pVtm_t->vlast);
		pVtm_t->vlast = voice;
...
by 3 db by simply limiting peak excursions whwen they occur */
		...
		if(pVtm_t->bDoTuning) {
			...
			if((pVtm_t->vtdTuneResults.purevmax > MAX_TRIGGER) || (pVtm_t->vtdTuneResults.rnzmax > MAX_TRIGGER) || (pVtm_t->vtdTuneResults.rnpmax > MAX_TRIGGER) || (pVtm_t->vtdTuneResults.c5max > MAX_TRIGGER) || (pVtm_t->vtdTuneResults.c4max > MAX_TRIGGER) || (pVtm_t->vtdTuneResults.c3max > MAX_TRIGGER) || (pVtm_t->vtdTuneResults.c2max > MAX_TRIGGER) || (pVtm_t->vtdTuneResults.c1max > MAX_TRIGGER)

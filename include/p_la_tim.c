...
void la_phtiming(LPTTS_HANDLE_T phTTS) {
  ...
  /* MAIN LOOP, for each output phoneme */
	...
	for(nphon = 0; nphon < pDph_t->nallotot; nphon++) {
		...
		//}
		if(((feacur & FVDSTOP) == FVDSTOP) && (pholas == GEN_SIL || strboulas >= FVPNEXT) && (pDphsettar->phonex_timing == LAP_O || pDphsettar->phonex_timing == LAP_U)) {
				...
				if((struccur & FTYPESYL) == FMONOSYL) {
					...
					if((strucstresscur & FSTRESS_1) IS_MINUS) {
						...
						if(strucstresscur IS_MINUS) {
							...
							//  of anything by las......
							if(pDph_t->allophons[nphon - 1] == LAP_L && pDph_t->allophons[nphon] == LAP_A && pDph_t->allophons[nphon + 1] != LAP_S) {
		...
		if((feacur & FCONSON) IS_PLUS) {
			...
			if(phocur == LAP_S) {
				...
				if(phocur == LAP_R) {
					if((strucboucur >= FMBNEXT) || ((pDphsettar->feanex & FCONSON) IS_PLUS))
						temp = N150PRCNT;
					else if(pholas == LAP_S || pholas == LAP_J || pholas == LAP_TH)
						temp = N60PRCNT;
				} else
				/* if (phocur == LAP_RR) */
				{
			...
			} else if((phocur == LAP_L || phocur == LAP_N) && (strucopensyl IS_PLUS))

			{
				 ...
				 */
				temp = N100PRCNT;
				if((pholas == LAP_N) && (feacur & FBURST) IS_PLUS)
					temp = N70PRCNT;
				else if(((feacur & FBURST) IS_PLUS) && (((fealas & FSONOR) IS_PLUS) && pholas != GEN_SIL) && (strucstresscur == FNOSTRESS))
					temp = N70PRCNT;
				else if((pDphsettar->phonex_timing == LAP_R || pDphsettar->phonex_timing == LAP_L) && (strucopensyl IS_PLUS))
					temp = N90PRCNT;
			...
			if((feacur & FNASAL) IS_PLUS) {
				if((phocur != LAP_NH) && ((struccur & FBOUNDARY) >= FVPNEXT)) {
					prcnt = mlsh1(N125PRCNT, prcnt);
